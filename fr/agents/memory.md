...
Dans un ancien chat de groupe Individual, confirme une fois les plages de connaissance manquantes des personnages. La première réplique d'un personnage ne prouve pas qu'il savait tout ce qui la précédait. Choisis un véritable personnage comme **Narrator** (Narrateur) seulement s'il doit dépasser les limites de participation. Le masquage propre à chaque personnage et les plages de connaissance confirmées limitent toujours la mémoire. Le masquage global **Hide from AI** (Masquer à l'IA), manuel ou défini par les résumés automatiques, retire seulement un tour de la transcription active : Advanced Memory continue de l'analyser, de déterminer les participants de la scène, de le résumer et de l'indexer pour les rappels autorisés. Les marqueurs de début raccourcissent la transcription active. Tu peux corriger ces plages plus tard ; chaque personnage ajouté doit avoir sa propre confirmation.

Pour un chat existant, clique d'abord sur **Prepare existing history** (Préparer l'historique existant). Si l'historique récupéré change les limites d'une scène couverte par une correction manuelle, désactive ce souvenir pour garder son texte comme référence, ou supprime-le, puis prépare de nouveau l'historique. Enregistrer la même correction ne permet pas d'attribuer son texte sans risque à une autre plage source. La préparation traite l'ancien historique par lots et affiche l'étape actuelle à côté de la roue de hamster de Professor Mari. **Cancel** conserve le travail terminé ; **Resume** reprend après la fermeture du panneau, le redémarrage du serveur ou la mise à jour de l'application. Un appel au modèle qui échoue conserve la mémoire précédemment valide et affiche une erreur pour réessayer. Ne réinitialise pas la mémoire pour te remettre d'un appel raté : Resume réutilise les résumés terminés et la détection des scènes inchangée. La dernière scène en cours reste ouverte et sera résumée à sa fermeture ; des extraits sources bornés sont utilisés si ses messages actifs dépassent la limite de contexte.
...
Ouvre **Access memories for this chat** (Accéder aux souvenirs de ce chat) dans le même panneau pour chercher les résumés de scènes numérotés chronologiquement, examiner leurs périodes et leurs publics, modifier **Summary text** (Texte du résumé) ou lire les messages originaux complets avec **Inspect source messages** (Examiner les messages sources). Les extraits internes reproduits mot pour mot ne sont pas des entrées de résumé de scène distinctes. Des périodes narratives fondées sur les sources accompagnent le contexte rappelé ; les dates inconnues restent inconnues. Tu peux désactiver des enregistrements de rappel, réindexer, exporter/importer, ou confirmer **Delete all memories** (Supprimer tous les souvenirs) pour recommencer la préparation en conservant le chat et ses réglages. Les corrections des résumés manuels originaux sont préservées et invalident la continuité qui en dépend. Pour exclure une scène du rappel, désactive son enregistrement de mémoire. Le masquage des sources propre à chaque personnage contrôle toujours qui peut y accéder ; le masquage global ne la retire pas de l'archive.

Un personnage présent pendant une partie d'une scène peut partager l'accès à son souvenir avec les autres participants, même si certains messages lui sont cachés. Une scène sans message source accessible reste indisponible. Les nouvelles instructions de résumé indiquent la visibilité des messages, décrivent les événements communs en texte simple et réservent `{{#if character == "Name"}}…{{/if}}` aux sections privées. Mentionner un personnage absent n'en fait pas un participant. Si la visibilité des sources ou les noms des personnages changent, l'ancien résumé n'est plus transmis aux lecteurs ayant un accès partiel jusqu'à ce que tu prépares à nouveau la scène ou vérifies ses conditions privées et enregistres le texte corrigé. Les anciens résumés sans informations de visibilité enregistrées nécessitent la même vérification avant un accès partiel ; changer uniquement les destinataires ne valide pas le texte. Les lecteurs ayant accès à tous les messages sources peuvent toujours rappeler ce souvenir, et le texte corrigé manuellement n'est jamais réécrit en silence. Ces vérifications n'appellent aucun modèle pendant le rappel. Les dates et périodes d'une scène sont des métadonnées communes à tous les participants assignés, même si certains messages sources sont cachés. Les extraits bruts ne contiennent jamais de messages cachés au personnage ; les extraits de résumés à portée conditionnelle restent réservés au narrateur, car la visibilité des messages ne suffit pas à décrire chaque fait privé. Les Chat Summaries constants continuent d'inclure les messages cachés à l'IA ; les conditions par personnage contrôlent l'accès aux détails privés sans exclure ces messages de la couverture du résumé.
