...
2. **World.** Définis le genre, le cadre, le ton, la difficulté, la classification du contenu et la langue. Choisis **Classic** (combat par menus) ou **Tactical** (combat sur grille) sous **Combat Preference** (préférence de combat). Les parties Tactical proposent aussi une graine, une taille et des indications de terrain facultatives. Si un ensemble de règles Game Mode est installé, **Rules** (Règles) apparaît dessous : garde **Marinara's own rules** ou choisis le ensemble de règles. Consulte [Choisir les règles](#choosing-rules).
...
L'assistant distingue trois cas : un combat résolu par l'ensemble de règles, un combat Marinara utilisant la santé, l'énergie et les capacités de la fiche, ou un combat Marinara suivant **Combat Preference**. Quand l'ensemble de règles résout le combat, le menu présente les attaques et capacités du personnage avec leur coût. Le panneau affiche l'ordre des tours, le round, les états et les budgets d'action restants ; le journal montre le calcul réel. Chaque action est immédiatement enregistrée dans la fiche, même si vous fermez l'onglet en plein combat. Le GM est ensuite informé que les chiffres sont déjà résolus. Si le ensemble de règles définit aussi la distance que représente une case, **Combat Preference** décide de la présentation : **Classic** joue sans positions et **Tactical** utilise un plateau généré où tu te déplaces, mesures les portées dans les unités de l'ensemble de règles et vises une case avec une zone. Consulte [Combat](combat.md#games-whose-ruleset-resolves-its-own-fights). Un ensemble de règles sans distance conserve la préférence, mais ne l'utilise pas.
...
Les chats Game exigent **Let the GM search lore** pour les recherches dans les lorebooks par outil, même quand **Enable Tool Use** est activé. Lorsque l'option est désactivée, `search_lorebook` est indisponible dans le sélecteur d'outils. Un élément déjà sélectionné reste visible avec une explication, pour que tu puisses le retirer ou réactiver la recherche de lore.
...
**Finish rolled turns in one request** (Terminer les tours avec des dés en une requête) change le second type d'appel décrit plus haut. Un tour avec des jets coûte normalement deux requêtes : le brouillon, puis une réécriture avec les vrais nombres. Avec cette option, le GM écrit l'issue à l'aveugle et le moteur remplit les nombres ensuite : le tour coûte une requête. Elle est désactivée par défaut. Deux choses ajoutent encore une requête : un appel à l'outil de dés avec **Enable Tool Use** actif et une **Game tool connection** distincte, qui fait toujours sa propre planification. Consulte [Game Mode : Dés et tests de compétence](dice-and-skill-checks.md#finishing-a-rolled-turn-in-one-request).

Si tes modèles locaux partagent une mémoire GPU limitée, ouvre **Chat Settings → Agents** et active **Run Game tasks one at a time** (exécuter les tâches Game une par une). La narration, les agents et les médias de scène s'attendent dans ce chat Game. L'option est désactivée par défaut et ne réserve pas le GPU face aux autres chats ou applications. Les commandes d'annulation existantes restent disponibles pendant l'exécution et l'attente.
