...
/**
 * Pins the background call cap from the environment: a positive integer sets the cap, "0" or
 * "off" disables it. The cap's own service parses it; listed here so the UI shows the lock.
 */
...
/** The saved value of a number setting, or its default. Environment overrides are parsed by the owning service. */
