...
async function readBackgroundMeta(): Promise<
  Record<string, { label?: string; originalName?: string; tags: string[] }>
> {
  if (!existsSync(BACKGROUND_META_PATH)) return {};
  try {
    return JSON.parse(await readFile(BACKGROUND_META_PATH, "utf8")) as Record<
      string,
      { label?: string; originalName?: string; tags: string[] }
    >;
  } catch {
    return {};
  }
...
async function writeBackgroundMeta(meta: Record<string, { label?: string; originalName?: string; tags: string[] }>) {
...
function appImagePathFromUrl(value: string): { path: string; label: string; url: string } | null {
  ...
  if (parts[1] === "gallery" && parts[2] === "file" && parts[3] && parts[4]) {
    ...
    return {
      path: assertInsideDir(GALLERY_DIR, join(GALLERY_DIR, chatId, filename)),
  ...
  if (parts[1] === "characters" && parts[3] === "gallery" && parts[4] === "file" && parts[2] && parts[5]) {
    ...
    return {
      path: assertInsideDir(root, join(root, filename)),
