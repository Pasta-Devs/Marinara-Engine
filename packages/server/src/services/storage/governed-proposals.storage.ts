...
export function createGovernedProposalStorage(db: DB) {
  type ProposalExecutor = Pick<DB, "select" | "insert" | "update">;

  return {
    async get(id: string, tx: ProposalExecutor = db) { return (await tx.select().from(statePatchProposals).where(eq(statePatchProposals.id, id)).limit(1))[0] ?? null; },
    async getByIdempotencyKey(key: string, tx: ProposalExecutor = db) { return (await tx.select().from(statePatchProposals).where(eq(statePatchProposals.idempotencyKey, key)).limit(1))[0] ?? null; },
    async listProposedByGroup(groupId: string, tx: ProposalExecutor = db) { return tx.select().from(statePatchProposals).where(and(eq(statePatchProposals.commitGroupId, groupId), eq(statePatchProposals.status, "proposed"))); },
    async propose(input: StatePatchProposalInput, tx: ProposalExecutor = db) {
      ...
      await tx.insert(statePatchProposals).values(row);
      return { status: "proposed" as const, row: row as typeof statePatchProposals.$inferSelect };
    },
    async resolve(id: string, status: Exclude<ProposalStatus, "proposed">, options: { diagnostic?: unknown; commitId?: string } = {}, tx: ProposalExecutor = db) {
      ...
      await tx.update(statePatchProposals).set({ status, diagnosticJson: options.diagnostic === undefined ? null : canonicalJson(options.diagnostic), commitId: options.commitId ?? null, resolvedAt: now() }).where(eq(statePatchProposals.id, id));
