...
export function ContextInjectionPanel({
  ...
  const directorSettings = useMemo(() => parseAgentSettings(directorConfig?.settings), [directorConfig?.settings]);
  ...
  const handleDirectorIntervalStep = useCallback(
    async (delta: number) => {
      if (!directorConfig || !directorIntervalMeta || updateAgent.isPending) return;
