  ...
  const filtered = useMemo(() => {
    ...
    return list.filter(
      (lb: LorebookListItem) =>
        ...
        lb.entitySummary.toLowerCase().includes(q) ||
        ...
        <div className="truncate text-[0.6875rem] text-[var(--muted-foreground)]">
          {characterName ? (
            <span className="inline-flex items-center gap-1">
              ...
              {characterName}
              {lorebook.entitySummary || lorebook.description
                ? localizeUi("ui.panels.lorebookrow.value1", { value1: lorebook.entitySummary || lorebook.description })
                : ""}
            </span>
          ) : (
            lorebook.entitySummary || lorebook.description || "No description"
          )}
        </div>
      </div>
      {!selectionMode && (
