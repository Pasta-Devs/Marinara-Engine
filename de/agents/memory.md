...
- **Maximum allowed context before compression (tokens)** (maximaler Kontext vor Komprimierung) begrenzt die vollständige Chat-Anfrage und jede Speicherverarbeitung, höchstens bis zum kleineren Kontextlimit des ausgewählten Modells. Geschätzte Prompt-Tokens, Werkzeuge, Anhänge, Antwortreserve und Sicherheitsspielraum zählen mit; dies ist kein exakter Token- oder Abrechnungszähler.
- **Maximum constant summary size (tokens)** (maximale Größe konstanter Zusammenfassungen) gilt nur für aktive Konstanten in **Chat Summaries** (Chat-Zusammenfassungen). Aktuelle Nachrichten, abgerufene Auszüge und archivierte Szenen zählen nicht. 8k Konstanten unter einer 10k-Grenze lösen durch angehängte Nachrichten keine Zusammenführung aus.
...
- Alle Speicherzusammenfassungen nutzen **Chat Summary → Maximum output size** (maximale Ausgabegröße), einschließlich Szenen und Konstantenzusammenführung. Das Ausgabelimit der Hilfsverbindung ersetzt diesen Wert nicht. Eingabe und Antwortreserve müssen gemeinsam unter die Kontextgrenze passen.
- Szenenzusammenfassungen erhalten Anweisungen, zulässige Nachrichten dieser Szene, passende bereichsbezogene Korrekturen und das JSON-Ausgabeformat. Zu große Szenen werden in gespeicherten Teilen verarbeitet und dann zusammengeführt. Der Standard-Prompt schreibt historische Rückblicke ohne aktuelle Situation oder offene Spannungen; eigene Prompts aus **Summaries** gelten weiterhin. Advanced Memory arbeitet unabhängig vom Hauptschalter Agents und benötigt keinen herunterladbaren Agenten.
...
Hauptgenerierung liest gespeicherte Erinnerung sofort. Sie startet keine Kontinuitätsgenerierung und wartet auch bei laufender Hintergrundarbeit nicht darauf. Erreicht die vollständige Anfrage die Grenze, verlassen ältere abgeschlossene Szenen das aktuelle Fenster. Passt eine laufende Szene oder übergroße Konstante nicht hinein, nutzt die Anfrage ausdrücklich markierte Quellauszüge und behält neueste Nachrichten. Gespeicherte Zusammenfassungen und Originale werden dabei nicht überschrieben.

Nach der Hauptantwort ergänzt Advanced Memory bereichsbezogene **Chat Summaries** nur um noch nicht erfasste archivierte Nachrichten und verwendet fertige Szenenrückblicke nach Möglichkeit wieder. Auch inaktive Einträge zählen als bearbeitete Bereiche. Überschreiten aktive Konstanten **Maximum constant summary size**, führt **Updating continuity** (Kontinuität aktualisieren) nach der Antwort ausschließlich deren Texte zusammen, mit Hilfsmodell und **Chat Summary → Maximum output size**. Der Ersatz erscheint in Chat Summaries; ersetzte Einträge werden inaktiv und bleiben wiederherstellbar. Szenen bleiben im Archiv. Fehler erhalten vorhandene Zusammenfassungen; **Resume processing** (Verarbeitung fortsetzen) wiederholt den Vorgang.

Abruf liefert relevante Szenenzusammenfassungen und genaue Dialoge mit Originalnummern und Sprechern. Jede Zusammenfassung steht bei ihrem Auszug mit einer gemeinsamen Bereichsüberschrift. Der Block nennt das aktuelle Verlaufsfenster und die letzte Nutzernachricht, um Vergangenheit und Gegenwart zu unterscheiden. Szenenrückblicke werden nur abgerufen, wenn sämtliche Quellen außerhalb des gesendeten Verlaufs liegen; Auszüge schließen aktuelle Nachrichten ebenfalls aus. Manuelle Bereichszusammenfassungen tragen nur bei, wenn ihr gesamter Bereich archiviert ist. Grenzüberschreitende Bereiche werden nicht vollständig neben aktuellen Quellen wiederholt. Persona- und Figurennachrichten zählen. Historische Neugenerierung verwendet Quellen vor der Zielantwort, auch vor dem verwalteten Fenster. Bearbeiten, Variantenwechsel, Verbergen oder Löschen von Quellen löst eine erneute Prüfung abgeleiteter Erinnerungen aus.

Öffne **Access memories for this chat** in derselben Seitenleiste, um chronologisch nummerierte Szenenzusammenfassungen zu durchsuchen, ihre Zeiträume und Empfängerkreise zu prüfen, **Summary text** (Zusammenfassungstext) zu bearbeiten oder mit **Inspect source messages** (Quellnachrichten prüfen) die vollständigen Originalnachrichten zu lesen. Interne wortgetreue Auszüge sind keine eigenen Szenenzusammenfassungen. Quellenbasierte Zeitangaben der Handlung begleiten den abgerufenen Kontext; unbekannte Daten bleiben unbekannt. Du kannst Abrufdatensätze deaktivieren, neu indexieren, exportieren/importieren oder **Delete all memories** (alle Erinnerungen löschen) bestätigen, um die Vorbereitung neu zu beginnen und dabei Originalchat und Einstellungen zu behalten. Korrekturen ursprünglicher manueller Zusammenfassungen bleiben erhalten und machen davon abhängige Kontinuität ungültig. Einen Datensatz zu deaktivieren ist etwas anderes, als seine Quellnachrichten auszublenden: Ausgeblendete Quellen bestimmen verbindlich das Charakterwissen.
