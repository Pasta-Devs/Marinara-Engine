...
2. **World.** Lege Genre, Schauplatz, Ton, Schwierigkeit, Inhaltsfreigabe und Sprache fest. Wähle unter **Combat Preference** (bevorzugte Kampfart) zwischen **Classic** (Menükämpfen) und **Tactical** (Rasterkämpfen). Tactical-Spiele bieten außerdem optionale Einstellungen für Schlachtfeld-Seed, Größe und Geländehinweise. Ist ein Game-Mode-Regelsatz installiert, erscheint darunter **Rules**: Behalte **Marinara's own rules** oder wähle den Regelsatz. Siehe [Regeln auswählen](#choosing-rules).
...
Der Assistent unterscheidet drei Wege: Kämpfe nach den eigenen Regeln des Regelwerks, Marinaras Kampf mit Gesundheit, Energie und Fähigkeiten aus dem Blatt oder Marinaras Kampf nach **Combat Preference**. Löst das Regelwerk den Kampf selbst auf, zeigt das Menü die Angriffe und Fähigkeiten der Figur samt Kosten. Die Anzeige nennt Zugreihenfolge, Runde, Zustände und verbleibende Aktionsbudgets; das Protokoll zeigt die tatsächliche Rechnung. Jede Aktion wird sofort ins Blatt geschrieben, auch wenn du den Tab mitten im Kampf schließt. Der GM erfährt danach, dass die Zahlen bereits feststehen. Deklariert der Regelsatz außerdem den Wert eines Spielfeldquadrats in seiner eigenen Entfernungseinheit, bestimmt **Combat Preference** die Darstellung: **Classic** verwendet keine Positionen, **Tactical** ein erzeugtes Spielfeld, auf dem du gehst, Reichweiten in den Einheiten des Regelsatzes misst und Formen auf Felder zielst. Siehe [Kampf](combat.md#games-whose-ruleset-resolves-its-own-fights). Ohne Entfernungsangaben bleibt die Präferenz gespeichert, wird aber nicht verwendet.
...
**Finish rolled turns in one request** (gewürfelte Züge mit einer Anfrage abschließen) ändert die zweite oben beschriebene Anfrageart. Gewöhnlich braucht ein Würfelzug zwei Anfragen: den Entwurf und die Umschreibung mit echten Zahlen. Bei aktivierter Option schreibt der Game Master den Ausgang blind und die Engine setzt die Zahlen danach ein; der Zug kostet eine Anfrage. Die Option ist standardmäßig aus. Zwei Dinge können weiterhin eine Anfrage ergänzen: ein Würfel-Tool-Aufruf bei **Enable Tool Use** und eine separate **Game tool connection**, die stets selbst plant. Siehe [Game Mode: Würfel und Fertigkeitsproben](dice-and-skill-checks.md#finishing-a-rolled-turn-in-one-request).
