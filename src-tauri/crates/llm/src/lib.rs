...
fn claude_subscription_prompt(request: &LlmRequest) -> ClaudeSubscriptionPrompt {
    ...
    let session_key = request
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let should_use_session = if marinara_runtime_metadata(&request.parameters).is_some() {
        claude_subscription_should_use_session(&request.parameters)
    } else {
        claude_subscription_resume_enabled() && session_key.is_some()
    };
    if should_use_session {
        if let Some(chat_id) =
            claude_subscription_chat_id(&request.parameters).or_else(|| session_key.map(str::to_string))
        {
...
mod tests {
    ...
    #[test]
    fn claude_subscription_session_key_fallback_uses_stable_session_prompt() {
        let request = LlmRequest {
            connection: test_connection(),
            session_key: Some("chat:chat-1:connection:default".to_string()),
        ...
        assert_eq!(prompt.prompt, "Next turn.");
        assert_eq!(prompt.prompt_shape, "trailing-user");
        let expected_session_id = claude_subscription_session_id("chat:chat-1:connection:default");
        assert_eq!(
            prompt.session_id.as_deref(),
            Some(expected_session_id.as_str())
        );
