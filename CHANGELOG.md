...
- Added OpenCode as a text connection provider. Marinara starts a local OpenCode server when needed, discovers models from the user's connected OpenCode providers, sends bounded tool-denied one-shot chat requests through the selected `provider/model`, honors debug-mode prompt logging for connection tests, and cleans up temporary sessions after each response or timeout.
