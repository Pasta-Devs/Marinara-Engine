...
- An opponent the Game Master plays, and the Engine's own choices for one, can now pay a Game Mode ruleset's ability out of a higher pool than it names: a spell cast out of a bigger slot, with whatever the ruleset says the extra buys. Only a player could do that before, so an opponent written with a spell that grows never grew it, on a turn or in a reaction alike. Each way of paying is weighed on its own, named for the pool it spends so the Game Master can tell two entries apart, and priced with the rungs it climbs so the bigger version is not mistaken for a free one (#6528).
