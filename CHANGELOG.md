...
- Added Mission Control as a Settings tab, with the Settings button carrying a count while work runs: follow complete chat missions through their Before, Reply, and After stages; inspect nested agent, memory, summary, and media work; safely stop every active mission; and review or clear the five most recent completed tasks and in-tab notifications. Standalone schedule generation, Professor Mari work, media/audio generation, backups, downloads, profile and SillyTavern imports, package installs, and updates appear with progress and context too. Chat missions stay compact, because the reply and the chat list already narrate them; background work shows its full stage path and child steps. Stop appears only where the engine can act on it: work that cannot be interrupted safely, such as a backup or a package install, says so rather than offering a button that does nothing.
