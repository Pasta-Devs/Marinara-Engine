...
- The roleplay Illustrator now sends native NovelAI per-character captions with positions when its image connection is NovelAI on a V4, V4.5, or V5 model: the prompt writer is asked for one caption per visible character, Marinara validates them against the scene's character list and forwards them as NovelAI character prompts, and the review-before-send dialog lists them read-only. The prompt writer copies Danbooru-tagged Appearance fields verbatim and converts prose or tracker outfits into tags, and with Attach Card Appearance on the writer receives every card's full Appearance text (ensemble `[NAME]` blocks split per character) as a reference and decides what belongs in each caption, so nothing is appended to the base prompt once captions exist. The caption cap follows the model (22 on V5, 6 on V4/V4.5) for storyboards too, and a V5 request that would have carried reference images now renders without them and logs a warning instead of failing, since NovelAI has not shipped Precise Reference for V5 yet.
