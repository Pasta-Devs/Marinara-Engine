...
- iPhone chat fields now keep Roleplay visible above the keyboard across both WebKit coordinate-space behaviors: the shell follows only the body displacement WebKit actually renders, so a `pageTop` update cannot push an already anchored page into the dark backing. Roleplay also releases its inactive full-resolution background after each crossfade and avoids unused full-viewport compositor hints, reducing persistent WebKit visual memory (#5710, #5711).
