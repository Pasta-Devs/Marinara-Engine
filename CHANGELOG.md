...
- Added Activity inside Settings, opened from a live button beside the panel's close button. The Settings button counts only work you are not already watching, such as backups, installs, or a reply in another chat, and a toast tells you when such work fails. Follow complete chat turns through their Before, Reply, and After stages; inspect nested agent, memory, summary, and media work; stop running work where the engine can act on it; and review or clear the five most recent completed tasks and in-tab notifications. Standalone schedule generation, Professor Mari work, media/audio generation, backups, downloads, profile and SillyTavern imports, package installs, and updates appear with progress and context too. The open chat's turn stays compact, because its reply already narrates it; other chats and background work show its full stage path and child steps. Stop appears only where the engine can act on it: work that cannot be interrupted safely, such as a backup or a package install, says so rather than offering a button that does nothing.
