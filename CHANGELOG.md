...
- The roleplay Illustrator now sends native NovelAI per-character captions with positions when its image connection is NovelAI on a V4, V4.5, or V5 model: the prompt writer is asked for one caption per visible character, Marinara validates them against the scene's character list and forwards them as NovelAI character prompts, and the review-before-send dialog lists them read-only. The prompt writer copies Danbooru-tagged Appearance fields verbatim and converts prose or tracker outfits into tags, and Attach Card Appearance now appends each character's Appearance text inside that character's caption instead of the shared base prompt, splitting ensemble cards' `[NAME]` blocks per character and skipping subject-count and duplicate tags. The caption cap follows the model (22 on V5, 6 on V4/V4.5) for storyboards too, and a V5 request that would have carried reference images now renders without them and logs a warning instead of failing, since NovelAI has not shipped Precise Reference for V5 yet.
