...
- iPhone chat fields now keep the app shell aligned with the visual viewport's absolute position, including when WebKit under-reports its keyboard offset, instead of leaving Roleplay on the dark page backing; Roleplay also releases its inactive full-resolution background after each crossfade and avoids unused full-viewport compositor hints, reducing persistent WebKit visual memory (#5710, #5711).
...
- Custom OpenAI-compatible connections targeting Z.Ai endpoints (and any provider serving the always-reasoning GLM 5.3 family, including plain `glm-5.3` and `glm-5.3-flash`) no longer send a reasoning-disable field that the endpoint rejects with "This model always engages in thinking" (#5582).
