...
- Added **Find in chat**, a search over the messages of the open Conversation or Roleplay with jump-to-message. Open it from the magnifying glass in the chat toolbar — kept out of the mobile overflow menu so it stays one tap on a phone — or with Ctrl/Cmd+K; browser find is deliberately left alone. Results list each match with its author, message number and surrounding text, so a word that appears many times can be recognised before jumping rather than stepped through blindly, and the panel stays open afterwards with prev/next arrows for walking the rest. On a phone the results sheet collapses to a compact stepper once you jump, leaving the chat visible. Matching is naive on purpose: this chat only, case-insensitive, literal text with no wildcards or operators, and it reaches messages far enough back that they have not loaded yet. Game mode is excluded, since it shows one narration beat at a time rather than a transcript and keeps its history in Session Logs.
