...
- **Keep generating when the tab is closed** (Settings > Advanced > Features, off by default): gallery images, selfies and scene videos, scene backgrounds, character art drafts and sprites keep generating on the server after the tab closes. Their status, a short log and the result are saved, returning to the app announces jobs that finished while you were away, and **Open generation jobs** under the switch lists recent jobs with a result preview and a Stop button. A restart marks unfinished jobs as interrupted instead of running them again. With the switch off nothing changes. See docs/development/generation-jobs.md.

- More server fixes from the same review: a crash between steps of emptying a table no longer brings deleted rows back from the pre-shard backup; a Windows clock adjustment no longer lets a second server take over a running one's storage; deleting a scene chat no longer clears another scene's pointer; chat, swipe and metadata routes answer 400 instead of 500 for missing bodies; conversation routes write only the keys they change; OOC influences are used up only when the reply is saved; conversation summaries stop when you stop the reply; tool-call markup a local model streamed is removed from the saved reply; stopping a raw request really stops it; Lorebook Keeper keeps separator lines and no longer drops facts found inside longer text; the custom lorebook backfill advances when proposals need approval; sprite uploads with a new file type replace the old sprite; SillyTavern and RisuAI imports keep regex scripts and outlet positions and report bad chat files as errors; a tool call inside a fenced block runs once; local model downloads and switches keep the installed model until the new one is ready; and outbound requests block reserved addresses even with a narrowed trusted network list and drop API keys on cross-site redirects.

- Server fixes from a review of services and storage: built-in regex scripts you deleted stay deleted; admin expunge and agent image routes answer 400 instead of 500 for missing bodies, and replaced agent images are removed; character export, embedded lorebook import and gallery uploads no longer lose data or leave partial files; knowledge-source uploads that fail or are cut off leave no file, and lorebook export and entry moves keep every book and entry; extension storage writes are serialized; agents no longer fall back to the default connection when they were skipped on purpose; long chats no longer slow down memory recall and prompt assembly; image-only history messages stay in the prompt; the autonomous scheduler serves every eligible chat instead of the first two; Discord webhook messages are retried after a 429; RunPod and ComfyUI jobs are cancelled when you stop; a failed primary connection no longer streams a second reply; saving app settings concurrently no longer fails; only language connections can be the default; Spotify playlists without a track count no longer break the tool; utility model reinstalls keep the working copy until the new download succeeds; Gemini Omni videos are no longer cut at the raw video size; and the Windows taskbar shortcut migration works on non-English paths.

- Server fixes from a review of the routes and middleware: a crash now saves pending changes before the server exits; saved request timeouts no longer make every `.env` edit ask for a restart; an IPv4 allowlist entry with a prefix over /32 is rejected instead of admitting almost everyone; IPv6 addresses are no longer all treated as local; per-route rate limits apply; background uploads with non-Latin or duplicate names no longer vanish or overwrite each other; the compatible profile export keeps items with the same name; large full backups can be exported and imported; replacing or deleting a connection image removes the old file; AVIF emojis and stickers import with their size; renaming a custom tool updates the agents that use it; parallel Google font downloads keep each other's entries; a failed selfie save no longer leaves orphan files; stopping a Peek Prompt dry run really stops it; Google Translate accepts long non-Latin text; and a non-JSON translation reply shows which service sent it.

- Models that always reason no longer fail when a request asks for reasoning off. If an OpenAI-compatible provider answers HTTP 400 that the model cannot disable reasoning (for example GLM 5.3 on NanoGPT), Marinara retries once without the reasoning-off setting, before any reply text arrived, and stops sending it to that model for the rest of the session.

- On Windows, start.bat opens the browser once the server answers instead of after a fixed 4 seconds, so a slow start (a large data folder, a first run) no longer lands on a connection error page. Without curl it keeps a short fixed delay.

- The launchers (start.bat, start.sh, start-termux.sh) now copy any untracked files in `packages/*/src` to `.tmp/untracked-src-backups/<time>/` before the startup `git clean` removes them, so a source file you had not committed yet can be recovered.

- **Background call cap** in Settings > Advanced > Features now works: when on, autonomous messages started by the server and other automatic model calls are limited to **Calls per hour** (600 by default) per rolling hour, refused without sending a request once the hour is spent. Off by default. `MARINARA_BACKGROUND_CALLS_PER_HOUR` still wins when set.

- Prompt debug output can go to files instead of the console: with `LOG_PROMPT_DEBUG_FILES=true`, prompts and model text from debug mode or `LOG_LEVEL=debug` are written to `logs/prompt-debug/` in the data folder, in size-limited files that rotate and prune themselves. Off by default. See docs/CONFIGURATION.md.

- The server now records what it was doing when memory runs short or it stalls. It samples heap, process, external and array buffer memory and the event-loop delay once a minute, and logs one warning when a pressure episode starts and one line when it ends. Stall lines include the same figures, plus counts from the storage, autonomous scheduler and local model workers. `MARINARA_RSS_WARN_MIB` sets the process memory warning level. `GET /api/admin/runtime-diagnostics` adds memory peaks since startup, the startup build check and the same worker counts.

- A stale or partial server build is reported instead of failing later with a confusing "Cannot find module": `pnpm build` fails when a source module has no compiled output (after one clean rebuild), startup warns with `ME_BUILD_STALE` when the running `dist` is missing modules or older than `src`, and the in-app updater no longer reports success for a rebuilt server that does not match the update. See docs/development/logging.md.
...
- Prompt-cache troubleshooting: with `MARINARA_CACHE_DIAGNOSTICS=1`, Claude (Subscription) and OpenAI Responses requests log hashes and counts (never prompt text) that show where two requests start to differ and what the provider reported as cached. With **Show token usage** on, a message now also shows what share of its input was served from the cache. With **Cache-friendly prompt layout** on, Peek Prompt's live preview shows the prompt the way the next real turn sends it, full lore first.

- New per-chat option **Warn before a low-cache send** in Chat settings > Advanced Parameters (off by default, warns below 80%). On Claude, Claude (Subscription), and ChatGPT with **Cache-friendly prompt layout**, a send whose prompt would mostly have to be cached again (a card or preset change near the top, or an expired cache) is held before anything reaches the model, and you choose whether to send it anyway. See docs/prompts/generation-parameters.md.

- With **Cache-friendly prompt layout** on, ChatGPT requests for the same chat now share one cache session (a `session-id` header and a `prompt_cache_key` derived from the chat id), and only the lore block is sent as instructions, so the provider can reuse the cached start of the prompt across turns. The **ChatGPT history replay** switch is gone: it had nothing to control in this version.

- New **Cache-friendly prompt layout** switch in Settings > Advanced > Features (off by default), for Claude (Subscription) and ChatGPT connections. On, a chat sends every enabled lorebook entry in its scope as one fixed `<lore>` block at the start of the prompt instead of the keyword scan's picks, with entries that use macros or decision blocks in a separate block near your message, and blocks that change every turn (recalled memories, the fallback chat summary, awareness and social activity) move next to your message, so providers can serve most of the prompt from their cache. A chat can keep the keyword scan with the metadata value `fullLorebookContext: false`. Off, prompts are exactly as before. See docs/configuration/features.md.
...
- A Game Mode party member you hand to the Engine to play can now pay a ruleset's ability out of a higher pool than it names: a spell cast out of a bigger slot, with whatever the ruleset says the extra buys. Only you could do that before, so a character played by the Engine never cast a spell that grows any bigger, on a turn or in a reaction alike. Each way of paying is weighed on its own and priced with the rungs it climbs, so the bigger version is not mistaken for a free one. Opponents are unchanged: a stat block has no pools to pay out of (#6528).
