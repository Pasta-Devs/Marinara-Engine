...
- Added Story Bundles, a new self-contained content type that packages a complete story setup — title, optional HTML description, characters, a persona, lorebooks, prompt presets, agents, and inline intro messages — into a single bundle. A new Story Bundles button in the TopBar opens a panel where bundles can be created, imported, exported, played, and deleted, and each bundle opens in a full-page editor with eight tabs: Metadata, Description, Characters, Personas, Lorebooks, Presets, Agents, and Intros.
- Added a Metadata tab to the Story Bundle editor with a bundle picture (drag & drop or file picker for JPG, PNG, or WebP, with avatar-crop preview), the read-only bundle ID, name, comment, creator, version, and free tags.
- Added a Description tab to the Story Bundle editor with an HTML description field and a preview toggle that renders the sanitized description live.
- Added character, lorebook, prompt preset, and agent assignment to Story Bundles: each tab offers search, a random-pick button, and paginated lists with add and remove controls; characters can additionally be added in bulk from character groups.
- Added inline intros to Story Bundles: named intro messages authored directly in the bundle's Intros tab (add, edit, delete). When playing a bundle that has intros, a choice dialog offers the intro names and the chosen intro becomes the first message of the new chat.
- Added a Play flow to Story Bundles: the Play button on a bundle row or in the editor header starts a new roleplay chat seeded with the bundle's characters, persona, prompt preset, and first active connection, activates the bundle's lorebooks and agents on the chat, and inserts the selected intro. If the selected preset has configurable variables, the choice-selection modal opens instead of the full setup wizard.
- Added export and import for Story Bundles: exporting downloads a self-contained `.marinara.json` with embedded characters, personas, lorebooks, and presets — including avatars, sprites, and gallery images — so a bundle can be transferred between PCs; importing creates a new bundle, skips embedded content that already exists by name, restores all binary data, and offers to install referenced agents that are not yet installed.
...
- Added a `manual-validation:story-bundle` pnpm script that runs the Story Bundle Playwright suite in a visible Chrome window, one test at a time, with a 1.5s slow-motion delay per action, for watching the flows by eye. The Playwright `slowMo` is now configurable via the `PLAYWRIGHT_SLOW_MO` environment variable (default 100ms).
...
- Fixed profile import rejecting backups that contain chat-gallery JSON manifests (such as `gallery/mari-images/manifest.json` from the Professor Mari preview gallery): gallery `.json` metadata files are now accepted during import instead of failing with "Profile asset … is not a supported image file", so exports from older versions import cleanly again.
- Fixed profile import rejecting backups that contain game-asset `.native` marker files (such as `game-assets/sprites/.native`): the empty markers written by game-asset seeding are metadata, never served to the browser, and recreated on every startup, so they are now accepted during import instead of failing with "Profile asset … is not a supported image file".
...
- Fixed the capability package catalog failing to load (HTTP 400) when a game-surface package declared package-owned static assets: the engine now accepts a `contributions.assets` field (a list of asset paths) instead of rejecting the whole catalog, so built-in agents such as Continuity Checker, World State, Prose Guardian, and Character Tracker can be installed and called again.
