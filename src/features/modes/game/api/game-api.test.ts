...
describe("gameApi.generateAssets prompt overrides", () => {
  beforeEach(() => {
    Object.values(storageApiMock).forEach((fn) => fn.mockReset());
  ...
  it("sends registered game image overrides to the provider for each game asset builder", async () => {
    mockGameAssetChat({
  ...
  it("keeps transient game image prompt review overrides ahead of registered defaults", async () => {
    mockGameAssetChat({
      "game.background": "REGISTERED BACKGROUND ${label} :: ${defaultPrompt}",
    });
