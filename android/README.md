...
- The Termux launcher sizes the Node.js heap from the structured profile size (roughly twice the on-disk profile plus headroom) bounded by device RAM, starting at a 1 GiB baseline. Advanced users can keep an explicit override, for example `NODE_OPTIONS="--max-old-space-size=1536" ./start-termux.sh`.
