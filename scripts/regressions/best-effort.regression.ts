...
assert.doesNotThrow(() => logSuppressed(new Error("cleanup failed"), { event: "regression.cleanup", chatId: "chat-1" }));
...
assert.equal(await orFallback(Promise.reject(new Error("read failed")), "fallback", { event: "regression.read" }), "fallback");
