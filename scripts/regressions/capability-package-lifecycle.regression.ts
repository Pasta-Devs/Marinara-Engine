...
try {
  ...
  assert.deepEqual(supportedCapabilityApi, { major: 1, minor: 39 });
  ...
  assert.match(
    ...
    /requires capability API 2\.0; this Engine supports 1\.39/,
  );
  ...
  const unsupportedMinorManifest = capabilityPackageManifestSchema.parse({
    ...manifestV2,
    capabilityApi: { major: 1, minor: 40 },
  });
  assert.match(
    getCapabilityApiCompatibilityIssue(unsupportedMinorManifest) ?? "",
    /requires capability API 1\.40; this Engine supports 1\.39/,
  );
