...
try {
  ...
  assert.deepEqual(supportedCapabilityApi, { major: 1, minor: 43 });
  ...
  assert.match(
    ...
    /requires capability API 2\.0; this Engine supports 1\.43/,
  );
  ...
  const unsupportedMinorManifest = capabilityPackageManifestSchema.parse({
    ...manifestV2,
    capabilityApi: { major: 1, minor: 44 },
  });
  assert.match(
    getCapabilityApiCompatibilityIssue(unsupportedMinorManifest) ?? "",
    /requires capability API 1\.44; this Engine supports 1\.43/,
  );
