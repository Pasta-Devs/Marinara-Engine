...
여러 사람을 설명하는 카드 하나, 예를 들어 등장인물이 여럿인 시나리오 카드에서도 각 인물을 별도로 추적할 수 있습니다. 카드 본문에서 각 인물에 `[CHARACTER: Name]` 제목이나 `Name:` 필드를 표시하면 트래커가 보고된 구성원을 해당 카드에 연결할 수 있습니다. 각 구성원은 자신의 이름, 기분, 의상, 생각, 초상화를 유지합니다. 카드 이름으로 된 이전 항목은 트래커가 이를 대체할 개별 구성원을 보고할 때까지 남습니다. 수동으로 추가한 캐릭터는 이름이 선언된 구성원과 같아도 수동으로 설정한 식별 정보를 유지합니다.

장면을 떠났던 캐릭터가 다시 등장하면 Character Tracker는 연속성을 위해 마지막으로 저장된 능력치와 사용자 지정 필드를 재사용합니다. 카드에 연결된 캐릭터에게는 설정된 RPG 자원과 속성도 기본 정보로 제공됩니다. 단일 캐릭터 카드 항목은 카드의 아바타와 자르기 설정을 유지합니다. 여러 캐릭터가 있는 카드의 구성원은 NPC와 동일한 라이브러리, 저장된 아바타 또는 선택적 초상화 생성 경로를 통해 각자의 초상화를 사용하며, 카드의 공통 초상화를 물려받지 않습니다.
