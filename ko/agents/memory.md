...
- **Maximum allowed context before compression (tokens)**(압축 전 최대 컨텍스트)는 전체 채팅 요청과 각 메모리 처리 요청에 적용되며 선택한 모델의 더 작은 컨텍스트 한도도 따릅니다. 추정 프롬프트 토큰, 도구, 첨부, 응답 예약 공간과 안전 여유를 포함합니다. 정확한 토큰 계산이나 요금 한도는 아닙니다.
- **Maximum constant summary size (tokens)**(상시 요약 최대 크기)는 **Chat Summaries**(채팅 요약)의 활성 상시 요약만 셉니다. 현재 메시지, 회상한 발췌문, 보관된 장면은 제외합니다. 한도 10k에 요약이 8k이면 첨부 메시지 때문에 통합하지 않습니다.
...
- 장면 요약과 상시 요약 통합을 포함한 모든 메모리 요약은 **Chat Summary → Maximum output size**(최대 출력 크기)를 씁니다. 보조 연결의 자체 한도로 대체하지 않습니다. 입력과 출력 예약 공간이 컨텍스트에 함께 들어가야 합니다.
- 장면 요약 요청에는 요약 지침, 해당 장면의 허용된 메시지, 적용할 범위별 수정과 JSON 출력 형식만 들어갑니다. 큰 장면은 저장되는 배치로 처리한 뒤 합칩니다. 기본 프롬프트는 현재 상황이나 열린 긴장 항목 없이 과거 사건을 요약하며 **Summaries**의 사용자 프롬프트도 적용됩니다. Advanced Memory는 Agents 주 스위치와 독립적이고 다운로드할 에이전트가 필요 없습니다.
...
장면 탐지는 Roleplay 주 응답을 저장한 뒤 실행합니다. **Standalone scene check interval (messages)**(독립 장면 확인 간격)는 기본 **5**입니다. 보조 모델은 최근 메시지 창, 장면 지침과 출력 형식만 받습니다. 페르소나와 캐릭터 메시지를 모두 셉니다. 트래커 에이전트와 그 일정에 독립적입니다. 장면 종료를 탐지한 경우에만 요약과 메시지 색인을 백그라운드에서 준비하며 불확실한 전환은 장면을 열어 두고 아카이브를 재구성하지 않습니다. 왼쪽 위 **Agents**에 **Advanced Recall**로 진행률, 오류와 복구 기능이 표시되며 일반 에이전트를 꺼도 보입니다. 진행 상태는 메모리 작업이 실행 중일 때만 주기적으로 조회하며, 준비된 아카이브는 유휴 상태에서 주기적으로 조회하지 않습니다.
...
주 생성은 저장 메모리를 즉시 읽으며 보조 모델이 실행 중이어도 연속성 생성을 시작하거나 기다리지 않습니다. 전체 요청이 한도에 도달하면 오래된 완료 장면을 현재 창 밖으로 옮깁니다. 열린 장면이나 큰 상시 요약이 들어가지 않으면 명시적으로 표시한 원문 발췌를 사용하면서 최신 메시지를 유지합니다. 이 조정은 저장 요약이나 원문을 덮어쓰지 않습니다.

주 응답 후 Advanced Memory는 아직 다루지 않은 보관 메시지만 범위가 있는 **Chat Summaries**에 추가하며 가능하면 완료된 장면 요약을 재사용합니다. 비활성 항목도 처리한 범위로 셉니다. 활성 상시 요약이 **Maximum constant summary size**를 넘으면 응답 후 **Updating continuity**(연속성 업데이트)가 그 요약 텍스트만 합칩니다. 선택한 보조 모델과 **Chat Summary → Maximum output size**를 씁니다. 대체 항목은 Chat Summaries에 나타나고 기존 항목은 비활성화되어 복구할 수 있습니다. 장면은 보관함에 남습니다. 실패하면 기존 요약을 유지하며 **Resume processing**(처리 재개)으로 재시도합니다.

회상은 관련 장면 요약과 원래 번호 및 화자가 있는 정확한 대화를 제공합니다. 요약 뒤 발췌문에 범위 제목을 한 번만 붙입니다. 현재 메시지 범위와 마지막 사용자 메시지 번호를 명시하여 과거와 현재를 구별합니다. 장면의 모든 원문이 전송 기록 밖에 있을 때만 회상하고 발췌문도 현재 메시지를 제외합니다. 수동 요약은 전체 범위가 보관된 경우에만 사용합니다. 경계를 가로지르는 범위는 현재 메시지 옆에 통째로 반복하지 않습니다. 페르소나와 캐릭터를 모두 셉니다. 과거 응답 재생성은 관리 창보다 이전이어도 대상보다 앞선 자료만 사용합니다. 원문 편집, 스와이프 변경, 숨기기나 삭제는 파생 메모리 재검사를 유발합니다.
...
| `recalled_messages` | 같은 통합 Recalled Scenes 섹션을 가리키는 기존 별칭. |

회상한 모든 장면은 하나의 **Recalled Scenes** 섹션에 표시하며, 각 요약 바로 뒤에 해당 장면의 발췌문이 있으면 붙입니다. 활성화된 `recalled_scenes` 마커가 기존 `recalled_messages` 마커보다 우선하며, 두 마커가 별도의 섹션을 만들지는 않습니다.
