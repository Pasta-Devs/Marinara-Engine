...
**Settings → Advanced → Request timeouts**에서 텍스트, 에이전트, Game 이미지 프롬프트, 이미지, 동영상, ComfyUI, 임베딩의 제한 시간을 초 단위로 조정하세요. 제한 시간을 늘리면 느린 로컬 백엔드가 작업을 마치는 데 도움이 되지만, 제공자가 자체적으로 적용하는 제한은 바꿀 수 없습니다. 저장하려면 서버 관리 권한이 필요합니다.

이 설정은 모든 프로필에 적용되며 활성 `.env` 옆의 `.env.timeouts.json`(사용자 지정 경로에서는 `<custom-env-path>.timeouts.json`)에 저장됩니다. `.env`를 다시 쓰지 않고 해당 환경 변수보다 우선 적용됩니다. 텍스트, 에이전트, Game 이미지 프롬프트, 임베딩 변경은 새 요청부터 적용됩니다. 미디어 변경과 설치된 에이전트 패키지에 반영하려면 서버를 다시 시작하세요. 환경 변수 설정으로 돌아가려면 타임아웃 설정 파일을 삭제하고 서버를 다시 시작하세요.
...
| `AUTO_OPEN_BROWSER` | `true` | 셸 런처가 앱 URL을 자동으로 열지 정합니다. `false`로 설정하면 끕니다. APK로 관리하는 설치에서는 셸의 브라우저 자동 열기를 끕니다. APK가 저장된 앱/브라우저 선택과 인증을 별도로 처리합니다. |
