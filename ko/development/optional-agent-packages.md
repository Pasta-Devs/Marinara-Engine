...
설치 후 Download Agents 상세 화면은 설치된 버전이 선언한 권한을 보여 줍니다. 카탈로그 버전이 다른 권한을 요청하면 별도로 표시합니다. 코드 설치나 업데이트에는 정확한 버전과 체크섬에 연결된 기존 승인이 계속 필요하며 모델 명령은 턴마다 별도 승인을 요청하지 않습니다.
