...
**Rules**(규칙)는 규칙 집합이 하나 이상 설치된 경우 새 게임을 만들 때만 표시됩니다. 판정과 캐릭터 시트의 규칙을 정하며, 전투 표시 방식을 고르는 **Combat Preference**와는 별개입니다. 기본값인 **Marinara's own rules**는 내장 d20 판정과 6개 능력치를 사용합니다. **5e (SRD 5.1)** 같은 규칙 집합은 지원 범위를 설명합니다. 전투를 지원하지 않으면 선택한 전투 표시 방식으로 Marinara Engine의 전투 규칙을 사용한다고 마법사가 안내합니다. 게임을 시작한 뒤에는 규칙 집합을 추가하거나 바꿀 수 없습니다.
