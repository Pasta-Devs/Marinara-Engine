...
UI 언어 팩은 별도의 수동 다운로드 절차를 사용합니다. 번역이 포함된 버전에서 처음 업데이트하면 인터페이스가 영어로 돌아갑니다. **Settings** → **General** → **Language**(언어)에서 언어를 다시 선택해 팩을 다운로드하세요. 이후 업데이트는 다운로드한 팩을 데이터 폴더에 유지합니다. 최신 번역을 받으려면 **Refresh language pack**(언어 팩 새로고침)을 사용하세요. 팩이 없거나 키가 번역되지 않았으면 영어가 표시되며, 시작할 때 UI 팩을 자동으로 다운로드하지 않습니다.
