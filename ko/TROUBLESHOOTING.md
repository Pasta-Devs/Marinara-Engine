...
### Termux: 클라이언트 빌드 중 메모리 부족

Vite가 `Reached heap limit` 또는 `JavaScript heap out of memory`로 종료되면 클라이언트 빌드에 필요한 Node.js 힙이 부족한 것입니다. Rollup 네이티브 바이너리 누락과는 다릅니다. 업데이트하고 `start-termux.sh`를 다시 실행하세요. 빌드는 작은 자동 힙 제한을 일시적으로 1536 MiB에 가깝게 높이되 알려진 기기 RAM의 절반을 넘지 않고 최소 1024 MiB를 유지합니다. 서버는 프로필 크기에 따른 더 작은 제한을 유지합니다. `NODE_OPTIONS`의 명시적 제한은 두 프로세스 모두에서 우선하므로 예전 1024 MiB 설정이 남았는지 확인하세요.

RAM에 따른 상한은 128 MiB 단위로 내림합니다. 기기 RAM의 절반이 1024 MiB보다 작으면 1024 MiB 하한이 우선합니다.

다시 시도하기 전에 다른 앱을 닫으세요. 메모리가 적은 기기는 여전히 실패하거나 Android가 프로세스를 종료할 수 있습니다. 이 경우 시작 출력 전체를 보관해 보고하세요. 빌드 실패를 고치려고 채팅이나 프로필을 삭제하지 마세요.
...
### Restart Server 후 서버가 돌아오지 않음

`start.bat`, `start-local.bat`, `start.sh`, `start-termux.sh` 또는 `pnpm start`로 Marinara를 시작하세요. 이 실행 방식은 서버를 실행 프로그램에 연결해 두고 이전 프로세스가 끝난 뒤 대체 프로세스를 시작합니다. 앱 내 재시작은 4초 후 남아 있는 연결을 닫고, 종료가 계속 멈춰 있으면 8초 후 강제 종료합니다. 강제 종료는 대기 중인 쓰기 작업을 중단할 수 있으며 진단에도 강제 종료로 기록됩니다. 직접 `node`로 실행한 프로세스와 개발 중 파일을 감시하는 프로세스는 자동으로 교체되지 않습니다. 터미널에서 중지한 뒤 다시 시작하세요. Docker는 기존 컨테이너 재시작 정책을 계속 사용합니다. Windows에서는 실행기 콘솔에서 Ctrl+C를 누르고 **Shutdown complete**(종료 완료)가 표시된 뒤 콘솔을 닫으세요. 터미널을 닫거나 강제 종료하면 대기 중인 저장 작업이 중단될 수 있습니다.

이전 서버가 실행 중일 때 같은 데이터 디렉터리를 사용하는 다른 서버를 시작하지 마세요. 이전 빌드가 프로세스를 남겼다면 먼저 해당 프로세스를 중지하세요. 실행 중인 서버의 쓰기 리스를 삭제하지 마세요.
...
Android 설정에서 Termux의 백그라운드 실행을 허용하고 배터리 최적화 대상에서 제외하세요. `termux-wake-lock`과 `termux-wake-unlock` 명령은 표준 Termux 설치라면 핵심 `termux-tools` 패키지에 모두 들어 있으므로 부가 기능을 따로 설치할 필요가 없습니다. 모든 제조사별 프로세스 종료를 막을 수는 없지만, 흔한 유휴 정지 원인을 없애고 영구 로그에 애플리케이션 수준 오류의 증거를 남길 수 있습니다.

### Termux를 포그라운드로 불러올 때까지 Marinara가 응답하지 않을 때

채팅이 "채팅 여는 중..."에서 멈추고 앱이 **Server unreachable**(서버에 연결할 수 없음)을 표시하며, Support Diagnostics(지원 진단 정보) 복사 내용의 서버 항목이 **Unreachable (request timed out)**(연결할 수 없음, 요청 시간 초과)로 나오고, Termux를 여는 순간 모든 것이 곧바로 정상으로 돌아온다면 호스트 프로세스는 크래시된 것이 아니라 **동결**된 상태입니다. Android의 캐시된 앱 동결 기능(그리고 일부 휴대폰에서 특히 공격적인 제조사별 대응 기능)이 Termux 프로세스 전체를 정지시킵니다. 휴대폰은 연결 자체는 받아들이지만 동결된 서버가 응답하지 않습니다. wake lock만으로는 프로세스가 동결에서 제외되지 않으며, 세션 로그에는 오류 대신 동결된 구간만큼 타임스탬프가 끊겨 있습니다.

이를 줄이려면 Android 설정에서 Termux를 배터리 최적화 대상에서 제외하고 백그라운드 활동을 허용하세요. 휴대폰이 지원한다면 최근 앱 화면에서 Termux를 고정하고, Termux 알림이 계속 보이게 두세요. 그래도 동결이 이어진다면 Marinara를 활발히 쓰는 동안 Termux를 포그라운드에 두거나 화면을 켜 두는 것이 확실한 우회 방법입니다.
