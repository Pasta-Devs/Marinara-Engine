...
MarinaraはNanoGPTの詳細なモデルカタログを読み、サブスクリプション対象のモデルと、より多くの割り当てを消費するモデルをドロップダウンに表示します。

- 対象モデルには**入力トークン倍率**のバッジが付きます。通常の消費率なら緑色の`1x`、送信トークン1個につき割り当てから複数の入力トークンを消費する場合は`2x`などの値です。対象*外*のモデルには**Paid**が付き、使用料は有料残高から差し引かれます。バッジがないモデルは、サブスクリプション情報を取得できなかったものです。

### サブスクリプション使用量

NanoGPTのサブスクリプションがある場合、この接続で現在の割り当てを確認できます。

1. 接続エディターで**Show subscription usage**(サブスクリプション使用量を表示)をオンにして保存してください。
2. 必要に応じて、先に**Management Token**(管理トークン)を貼り付けてください。

ウィジェットはNanoGPTの使用量エンドポイントを読み、週ごとと日ごとの入力トークン割り当てを表示します。次の2種類の認証情報を使えます。

- **Management Token**(推奨): `https://nano-gpt.com/settings#management-api-tokens`で、**Usage only**(使用量のみ)のスコープを持つトークンを作成してください。割り当てを読めますが、モデルの実行や残高の消費はできないため、使用量表示にはこちらが安全です。Marinaraは暗号化して保存し、NanoGPTの管理エンドポイントだけに送信します。
- **APIキー**: 管理トークンが保存されていない場合に自動で使います。これでも動作しますが、割り当ての読み取りに残高を消費できる認証情報を使うことになります。

NanoGPTが現在報告できない割り当ては、未使用ではなく**不明**として表示します。無効なサブスクリプションはその状態を明示し、全量が利用可能であるかのようには表示しません。更新ボタンで必要なときにウィジェットを更新できます。

同じメーターはチャットの接続選択にあるコンテキスト表示の下と、チャットの**Connection**(接続)設定にも表示されるため、チャットを離れずに割り当てを確認できます。アクティブな接続に連動し、NanoGPT接続で**Show subscription usage**がオンの場合だけ表示します。問い合わせる接続を1つに特定できない**Random**では表示しません。接続選択では上のコンテキストバーに合わせた外観になり、読み方も同じです。週の割り当てに対する使用量を、たとえば`8.9M / 60M tokens`と表示します。
