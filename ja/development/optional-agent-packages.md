...
インストール後のDownload Agentsの詳細画面には、インストール済みバージョンが宣言した権限が表示されます。カタログ側のバージョンが異なる権限を要求する場合は、分けて表示します。コードのインストールや更新には、そのバージョンとチェックサムに厳密に結び付いた既存の承認が引き続き必要です。モデルコマンドが毎ターン個別の承認を求めることはありません。
