...
Illustratorの画像接続がNovelAIのV4、V4.5、V5モデルを使う場合、プロンプトを書くモデルには、画面に登場するキャラクターごとに追加の記述を作るよう指示されます。各記述には、そのキャラクターの外見、表情、ポーズ、画面内のおおよその位置が含まれます。Marinaraは記述をシーンのキャラクター一覧と照合し、対応するキャラクターが見つからないものを除外して、残りをNovelAI標準のキャラクタープロンプトとしてシーンのメインプロンプトと一緒に送信します。これにより、複数のキャラクターがいるシーンで髪や服などの特徴が混ざるのを防ぎます。

記述できる人数はモデルによって異なります。V5は最大22人、V4とV4.5は最大6人です。それ以上のキャラクターが登場するシーンでは、重要なキャラクターを優先し、残りは名前のない背景の人物として扱います。

プロンプトを書くモデルには、各記述の詳細をどこから取得するかも指示されます。変わらない特徴は、キャラクターやペルソナの**Appearance**(外見)フィールドから取得します。Danbooruタグで書かれていればそのままコピーし、文章ならタグに変換します。服装は、キャラクタートラッカーが動作している場合、その時点の服装から取得してタグに変換します。

**Attach Card Appearance**(カードの外見を添付)をオンにすると、チャット内の各カードとペルソナのAppearance全文も参照用に渡されるため、長いカードや複数のキャラクターを含むカードが途中で切り詰められません。1つのAppearanceフィールドで`[NAME] tags | [NAME] tags`の形式で複数のキャラクターを記述したカードは、キャラクターごとに分けて渡されます。何を使うかはモデルが判断します。変わらない特徴は対応するキャラクターの記述にコピーし、カード内の服装タグはトラッカーやシーンで置き換えられるデフォルトとして扱い、シーンにいないキャラクターは無視します。キャラクターの記述が返された場合、Marinaraはプロンプトに何も追加しません。1人だけのシーンのように記述が返されなかった場合は、従来どおり外見の行をメインプロンプトに追加します。

特別な設定は不要です。この記述はNovelAI自身のホストへの接続でのみ使われるため、プロキシやほかのプロバイダーには影響しません。**Review image prompts before sending**(送信前に画像プロンプトを確認)をオンにすると、確認ダイアログのメインプロンプトの下に記述が表示され、送信内容を確認できます。記述はそこで編集できませんが、メインプロンプトは通常どおり編集できます。
