...
- Temperament uses **tempérament**; a ruleset uses **ensemble de règles**. Keep capability,
  combat role, temperament and controller distinct (AI design, section 3).
- Legendary actions use **actions légendaires**; triggered reactions use **réactions**
...
- Use **modèle de décision** for the concept and preserve the UI label **Decision model** with that gloss on first use.
- Use **énoncé de décision** for a statement sent to the model. An agent activation question and a statement inside its prompt are distinct: the former gates execution, the latter selects prompt text.
- Use **piste de blessures** for a ruleset wound track. Keep `levels`, `kinds`, damage kinds and ordinary numeric tracks distinct.
