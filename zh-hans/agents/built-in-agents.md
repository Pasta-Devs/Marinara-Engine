...
一张描述多个人物的角色卡，例如包含一组登场人物的场景卡，也能分别跟踪他们。在卡片文本中，用`[CHARACTER: Name]`标题或`Name:`字段标记每个人物，让跟踪器将报告的成员关联到对应卡片。每个成员保留自己的姓名、心情、服装、想法和头像。以卡片名称命名的旧条目会保留，直到跟踪器报告一个具体成员来替换它。手动添加的角色即使与声明的成员同名，也会保留手动设定的身份。

已离开场景的常驻角色再次出现时，Character Tracker会复用其最近保存的数值和自定义字段，保持连贯性。有角色卡的角色还会获得已配置的RPG资源池和属性作为依据。单角色卡条目保留卡片的头像和裁剪设置。多角色卡的成员使用各自的头像，来源与NPC相同，可以是图库、已保存的头像或可选的头像生成流程；他们不会继承卡片的共用头像。
