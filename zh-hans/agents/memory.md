...
普通召回读取已准备的记忆。可选的查询嵌入有较短超时，不可用时回退到文本匹配。文本匹配会提高最新用户消息中特征词的权重，让简短的细节也能找到较长的场景摘要。即使关闭片段输出，也能通过已索引的原始消息找到场景。这两项都不会增加模型调用。召回仅用于 Roleplay 主生成：智能体调用、手动重跑和辅助试运行不会触发它，也不会接收返回的摘要或片段。主提示词检查保持只读。
...
只参加了部分场景的角色也可以与其他参与者共享该场景的记忆访问权限，即使其中有些消息对该角色隐藏。没有任何可访问原始消息的场景仍不可用。新的摘要指令会说明消息可见性，用普通文字描述共同经历，仅对私密部分使用 `{{#if character == "Name"}}…{{/if}}`。提到不在场的角色不会使其成为参与者。如果来源可见性或角色名称改变，旧摘要会暂时对仅有部分访问权限的读者隐藏，直到你重新准备该场景，或检查其私密条件并保存修正后的文本。未保存可见性信息的旧摘要也需要经过同样检查，才能允许部分访问；仅修改受众不代表已确认文本内容。能访问全部原始消息的读者仍可召回该记忆，手动修正的文本也绝不会被悄悄重写。这些检查不会在召回期间调用模型。场景日期和时间范围是所有已分配参与者共享的元数据，即使部分原始消息被隐藏也是如此。原文片段绝不包含对角色隐藏的消息；带条件范围的摘要所附片段仍仅限叙述者使用，因为仅靠消息可见性无法描述每一项私密事实。常驻 Chat Summaries 仍包含对 AI 隐藏的消息；角色条件控制私密细节的访问权限，不会将这些消息排除在摘要覆盖范围之外。
