...
请使用 `start.bat`、`start.sh`、`start-termux.sh` 或 `pnpm start` 启动 Marinara。这些方式会让服务器保持受启动器管理，并等旧进程退出后再启动替代进程。从应用内重启时，会在 4 秒后关闭仍未结束的连接；如果关闭过程仍卡住，则在 8 秒后强制退出。强制关闭可能中断待完成的写入，并会在诊断中标记为强制关闭。直接使用 `node` 启动的进程和开发模式下监视文件的进程不会自动替换：请在终端中停止后重新启动。Docker 继续使用容器的重启策略。
