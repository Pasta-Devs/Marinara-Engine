...
界面语言包采用单独的手动下载流程。首次从内置翻译的版本升级时，界面会恢复为英语；请在 **Settings** → **General** → **Language**(语言)中重新选择你的语言来下载语言包。后续更新会保留数据文件夹中已下载的语言包。使用 **Refresh language pack**(刷新语言包)获取新翻译。语言包缺失或键未翻译时会显示英语，启动时不会下载界面语言包。
