...
本指南介绍 Marinara Engine 里的生成参数。这些设置决定 AI 每次回复的写法，比如 **Temperature**(温度) 和 **Max Output Tokens**(最大输出 Token 数)。预设和连接提供默认值；**Advanced Parameters**(高级参数) 面板用于按聊天覆盖这些值。
...
下表列出参数编辑器显示的回退值，以及 Send 开关的默认状态。这些值不一定就是发送给模型的值：没有预设时，生成从 `4096` 个输出 Token 开始，再应用连接和聊天的覆盖值。之后，模式规则可将活动场景设为 `8192`，将 Game 设为 `16384`。**Effective** 一行显示最终解析出的值。
