...
下表列出参数编辑器显示的回退值，以及 Send 开关的默认状态。这些值不一定就是发送给模型的值：没有预设时，生成从 `4096` 个输出 Token 开始，再应用连接和聊天的覆盖值。活动场景可以使用 `8192`。Game 使用连接和聊天保存的参数，只为未配置的值采用任务默认值。**Effective** 一行显示最终解析出的值。

| 参数 | 起始值 | 默认发送 |
|---|---|---|
| Temperature | 1 | 否 |
| Max Output Tokens | Conversation 为 4096，Roleplay 为 8192；Game 不覆盖此值 | 是 |
| Top P | 1 | 否 |
| Top K | 0(关闭) | 否 |
| Frequency | 0 | 否 |
| Presence | 0 | 否 |
...
4. 模式规则：正在进行的场景聊天将输出设为 `8192`、推理设为 **Maximum**、详细程度设为 **High**。Game 使用连接和聊天保存的参数，不强制设置采样、推理或最小输出长度。辅助调用也遵循保存的参数，只有未配置的值才采用任务默认值。
5. 输出限制：Game 应用模型已知的输出上限，连接的 **Max Output Tokens override** 在所有模式下限制请求。可用上下文还可能进一步减少输出预算。
