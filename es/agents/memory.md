...
En un chat grupal Individual antiguo, confirma una vez los intervalos de conocimiento que falten para cada personaje. La primera intervención de un personaje no demuestra que conociera todo lo anterior. Selecciona un personaje real como **Narrator** (Narrador) solo si debe saltarse los límites de participación. La ocultación específica por personaje y los intervalos de conocimiento confirmados siguen restringiendo la memoria. **Hide from AI** (Ocultar a la IA) global, tanto manual como establecido por resúmenes automáticos, solo quita un turno de la transcripción activa: Advanced Memory sigue analizándolo, identificando a los participantes de la escena, resumiéndolo e indexándolo para la recuperación permitida. Los marcadores de inicio recortan la transcripción activa. Puedes corregir estos intervalos después; los personajes que añadas necesitan su propia confirmación.

En un chat existente, haz clic primero en **Prepare existing history** (Preparar historial existente). Si el historial recuperado cambia los límites de una escena cubierta por una corrección manual, desactiva esa memoria para conservar su texto como referencia, o elimínala, y vuelve a preparar el historial. Guardar la misma corrección no puede asignar su texto de forma segura a otro intervalo de origen. La preparación procesa el historial antiguo por lotes y muestra su fase actual junto a la rueda de hámster de Professor Mari. **Cancel** conserva lo completado; **Resume** continúa después de cerrar el panel, reiniciar el servidor o actualizar la app. Una llamada al modelo fallida conserva la memoria válida anterior y muestra un error para reintentar. No restablezcas la memoria para recuperarte de una llamada fallida: Resume reutiliza los resúmenes completados y la detección de escenas que no haya cambiado. La última escena en curso permanece abierta y se resume al cerrarse; si sus mensajes activos superan el límite de contexto, se usan fragmentos acotados de origen.
...
Abre **Access memories for this chat** (Acceder a las memorias de este chat) en el mismo panel para buscar resúmenes de escenas numerados cronológicamente, inspeccionar sus períodos y audiencias, editar **Summary text** (Texto del resumen) o leer los mensajes originales completos con **Inspect source messages** (Inspeccionar mensajes de origen). Los fragmentos literales internos no son entradas de resumen de escena separadas. Los períodos narrativos basados en las fuentes acompañan al contexto recuperado; las fechas desconocidas siguen siendo desconocidas. Puedes desactivar registros de recuperación, reindexar, exportar/importar o confirmar **Delete all memories** (Eliminar todas las memorias) para reiniciar la preparación conservando el chat original y sus ajustes. Las correcciones de los resúmenes manuales originales se conservan e invalidan la continuidad dependiente. Para excluir una escena de la recuperación, desactiva su registro de memoria. La ocultación de origen específica por personaje sigue controlando quién puede acceder a ella; la ocultación global no la quita del archivo.

Un personaje presente durante parte de una escena puede compartir el acceso a su recuerdo con otros participantes, aunque algunos mensajes estén ocultos para él. Una escena sin mensajes de origen accesibles sigue sin estar disponible. Las nuevas instrucciones de resumen indican la visibilidad de los mensajes, describen los hechos compartidos en texto normal y reservan `{{#if character == "Name"}}…{{/if}}` para las secciones privadas. Mencionar a un personaje ausente no lo convierte en participante. Si cambia la visibilidad de las fuentes o los nombres de los personajes, el resumen anterior se retiene para quienes tienen acceso parcial hasta que prepares de nuevo la escena o revises sus condiciones privadas y guardes el texto corregido. Los resúmenes antiguos sin información de visibilidad guardada necesitan la misma revisión antes de permitir acceso parcial; cambiar solo los destinatarios no confirma el texto. Quienes pueden acceder a todos los mensajes de origen aún pueden recuperar el recuerdo, y el texto corregido manualmente nunca se reescribe sin avisar. Estas comprobaciones no llaman a un modelo durante la recuperación. Las fechas y los periodos de la escena son metadatos compartidos por todos los participantes asignados, incluso cuando algunos mensajes están ocultos. Los fragmentos originales nunca incluyen mensajes ocultos para el personaje; los fragmentos de resúmenes con condiciones siguen limitados al narrador porque la visibilidad de los mensajes por sí sola no puede describir todos los datos privados. Los Chat Summaries constantes siguen incluyendo los mensajes ocultos para la IA; las condiciones por personaje controlan el acceso a los detalles privados sin excluir esos mensajes de la cobertura del resumen.
