...
- **Maximum allowed context before compression (tokens)** (contexto máximo antes de comprimir) limita la solicitud completa del chat y cada procesamiento de memoria, respetando el límite menor del modelo elegido. Incluye tokens estimados del prompt, herramientas, adjuntos, reserva de respuesta y margen de seguridad; no es un contador exacto ni un límite de facturación.
- **Maximum constant summary size (tokens)** (tamaño máximo de resúmenes constantes) solo cuenta constantes activos en **Chat Summaries** (resúmenes del chat), no mensajes actuales, fragmentos recuperados ni escenas archivadas. Unos constantes de 8k bajo un límite de 10k no se consolidan por adjuntar mensajes.
...
- Todas las llamadas de resumen usan **Chat Summary → Maximum output size** (tamaño máximo de salida), incluidas escenas y consolidación de constantes. La conexión auxiliar no sustituye este valor por su propio límite. El contexto debe admitir entrada y reserva de salida.
- Cada resumen de escena recibe instrucciones, mensajes autorizados de esa escena, correcciones aplicables por rango y formato JSON de salida. Las escenas grandes se procesan en lotes guardados y luego se combinan. El prompt predeterminado crea relatos históricos sin situación actual ni tensiones abiertas; siguen aplicándose los prompts personalizados de **Summaries**. Advanced Memory funciona independientemente del interruptor principal Agents y no necesita un agente descargable.
...
La generación principal lee memoria guardada inmediatamente. Nunca inicia ni espera generación de continuidad, aunque trabaje un auxiliar. Al alcanzar el límite de la solicitud completa, las escenas antiguas terminadas salen de la ventana activa. Si una escena abierta o constante demasiado grande no cabe, usa fragmentos de origen identificados y conserva los últimos mensajes. El ajuste no sobrescribe resúmenes ni originales.

Después de la respuesta principal, Advanced Memory amplía **Chat Summaries** por rangos solo con mensajes archivados no cubiertos, reutilizando escenas cuando puede. Las entradas inactivas también cuentan como rangos atendidos. Si los constantes activos superan **Maximum constant summary size**, **Updating continuity** (actualizando continuidad) combina después de la respuesta solo sus textos, usando el auxiliar y **Chat Summary → Maximum output size**. El reemplazo aparece en Chat Summaries; las entradas sustituidas quedan inactivas y recuperables. Las escenas siguen en la bóveda. Un fallo conserva los resúmenes; reintenta con **Resume processing** (reanudar procesamiento).

El archivo recupera escenas relevantes y diálogos exactos con números originales y hablantes. Cada resumen acompaña a su fragmento con una sola cabecera de rango. El bloque indica el rango actual y el último mensaje del usuario para distinguir pasado y presente. Una escena solo se recupera si todas sus fuentes quedan fuera del historial enviado; los fragmentos también excluyen mensajes actuales. Los resúmenes manuales aportan continuidad solo si todo su rango está archivado. Un rango que cruza el corte no se repite entero junto a sus mensajes actuales. Cuentan personas y personajes. La regeneración histórica usa fuentes anteriores al destino, incluso si precede a la ventana gestionada. Editar, cambiar de variante, ocultar o borrar fuentes obliga a comprobar la memoria derivada.
