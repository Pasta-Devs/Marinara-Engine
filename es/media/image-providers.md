...
Servicio en la nube con la Base URL predeterminada `https://api.atlascloud.ai/api/v1`. Necesita una clave API de Atlas Cloud. **Fetch Models** (Obtener modelos) carga el catálogo actual de modelos de texto a imagen y de imagen a imagen de Atlas Cloud. Si no se puede acceder al catálogo, Marinara muestra una pequeña lista inicial con Nano Banana, Gemini Flash Image y FLUX 1.1 Pro. También puedes escribir el identificador exacto de cualquier modelo de imagen de Atlas Cloud. Los trabajos son asíncronos: Marinara inicia la generación y consulta Atlas Cloud hasta que la imagen está lista. Los controles habituales de texto a imagen se asignan automáticamente; las imágenes de referencia se envían a los modelos cuyos identificadores indican funciones de imagen a imagen, edición o Kontext. Como los esquemas pueden variar, consulta la documentación de Atlas Cloud del modelo elegido si usas otro identificador.
