...
2. **World.** Define el género, la ambientación, el tono, la dificultad, la clasificación de contenido y el idioma. Elige **Classic** (Combate con menús) o **Tactical** (Combate en cuadrícula) en **Combat Preference** (Preferencia de combate). Las partidas Tactical también ofrecen semilla, tamaño e indicaciones de terreno opcionales. Si hay un conjunto de reglas Game Mode instalado, aparece **Rules** (Reglas) debajo: conserva **Marinara's own rules** o elige el conjunto. Consulta [Elegir las reglas](#choosing-rules).
...
El asistente distingue tres opciones: combates resueltos por el propio reglamento, combate de Marinara con la salud, energía y habilidades de la ficha, o combate de Marinara según **Combat Preference**. Si el reglamento resuelve el combate, el menú muestra los ataques y habilidades del personaje con sus costes. El panel presenta el orden de turnos, la ronda, los estados y los recursos de acción restantes; el registro muestra el cálculo real. Cada acción se guarda inmediatamente en la ficha, incluso si cierras la pestaña a mitad del combate. Después, el GM recibe la indicación de que las cifras ya están resueltas. Si el conjunto también define cuánto representa una casilla en su propia unidad de distancia, **Combat Preference** determina la presentación: **Classic** juega sin posiciones y **Tactical** usa un tablero generado donde te mueves, mides alcance y distancia en las unidades del conjunto y apuntas un área a una casilla. Consulta [Combate](combat.md#games-whose-ruleset-resolves-its-own-fights). Un conjunto sin distancia conserva la preferencia, pero no la usa.
...
Los chats Game requieren **Let the GM search lore** para consultar lorebooks mediante herramientas, incluso con **Enable Tool Use** activado. Mientras esté desactivado, `search_lorebook` no estará disponible en el selector de herramientas. Una entrada ya seleccionada permanece visible con una explicación para que puedas quitarla o volver a activar la búsqueda de lore.
...
**Finish rolled turns in one request** (Terminar turnos con dados en una sola solicitud) cambia el segundo tipo de llamada descrito arriba. Un turno con tiradas normalmente cuesta dos solicitudes: el borrador y una reescritura con las cifras reales. Al activarlo, el GM escribe el desenlace a ciegas y el motor añade los números después, por lo que cuesta una solicitud. Está desactivado de forma predeterminada. Dos cosas todavía añaden otra solicitud: llamar a la herramienta de dados con **Enable Tool Use** activado y una **Game tool connection** separada, que siempre hace su propia planificación. Consulta [Game Mode: Dados y pruebas de habilidad](dice-and-skill-checks.md#finishing-a-rolled-turn-in-one-request).

Si tus modelos locales comparten una memoria GPU limitada, abre **Chat Settings → Agents** y activa **Run Game tasks one at a time** (ejecutar tareas Game de una en una). La narración, los agentes y los medios de la escena esperan unos a otros dentro de ese chat Game. La opción empieza desactivada y no reserva la GPU frente a otros chats o aplicaciones. Los controles de cancelación existentes siguen disponibles durante la ejecución y la espera.
