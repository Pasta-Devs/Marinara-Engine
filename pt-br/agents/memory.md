...
- **Maximum allowed context before compression (tokens)** (contexto máximo antes da compressão) limita a solicitação completa e cada processamento de memória, respeitando o contexto menor do modelo escolhido. Inclui prompt estimado, ferramentas, anexos, reserva de resposta e margem de segurança; não é contagem exata nem limite de cobrança.
- **Maximum constant summary size (tokens)** (tamanho máximo dos resumos constantes) conta apenas constantes ativos em **Chat Summaries** (resumos do chat), sem mensagens atuais, trechos recuperados ou cenas arquivadas. Constantes de 8k sob limite de 10k não são consolidados por mensagens anexadas.
...
- Todos os resumos de memória, incluindo cenas e constantes, usam **Chat Summary → Maximum output size** (tamanho máximo da saída). A conexão auxiliar não substitui esse valor pelo próprio limite. Entrada e reserva de saída devem caber no contexto.
- Cada resumo de cena recebe instruções, mensagens permitidas daquela cena, correções aplicáveis por intervalo e formato JSON de saída. Cenas grandes são processadas em lotes salvos e depois combinadas. O prompt padrão produz relato histórico sem situação atual ou tensões abertas; prompts personalizados de **Summaries** continuam valendo. Advanced Memory independe do interruptor principal Agents e não exige baixar um agente.
...
Em chats existentes, clique em **Prepare existing history** (preparar histórico existente). O processamento ocorre em lotes, com a etapa junto à roda de Professor Mari. **Cancel** (cancelar) preserva trabalho concluído; **Resume** (retomar) continua após fechar, reiniciar o servidor ou atualizar. Falhas preservam memória válida e permitem nova tentativa. Não redefina a memória: retomar reutiliza resumos prontos e detecção sem alterações. A última cena fica aberta até terminar; se suas mensagens excederem o contexto, são usados trechos de origem limitados até o resumo final.
...
A detecção ocorre após salvar a resposta principal de Roleplay. **Standalone scene check interval (messages)** (intervalo de verificação independente) é **5** por padrão. O auxiliar recebe apenas a janela recente, instruções de cena e formato de saída. Contam personas e personagens. A verificação independe dos agentes de acompanhamento e seus agendamentos. Só um final detectado prepara resumo e índice em segundo plano; transições incertas mantêm a cena aberta sem reconstruir o arquivo. **Agents**, no canto superior esquerdo, mostra **Advanced Recall**, com progresso, erros e recuperação, mesmo com agentes comuns desligados. O progresso é consultado periodicamente apenas enquanto uma tarefa de memória está em execução; arquivos prontos não são consultados periodicamente durante a inatividade.

A recuperação lê memória preparada. O embedding opcional da consulta tem prazo curto e recorre à correspondência textual se indisponível. Só funciona na geração principal de Roleplay: agentes, reexecuções manuais e gerações auxiliares de teste não a acionam nem recebem seus resumos ou trechos. A inspeção do prompt principal continua somente leitura.
...
A geração principal lê memória salva imediatamente, sem iniciar ou esperar geração de continuidade, mesmo com auxiliar trabalhando. Quando a solicitação completa atinge o limite, cenas antigas concluídas saem da janela ativa. Se uma cena aberta ou constante grande não couber, usa trechos de origem identificados e mantém as últimas mensagens. Esse ajuste não sobrescreve resumos nem originais.

Após a resposta principal, Advanced Memory amplia os **Chat Summaries** por intervalos apenas para mensagens arquivadas não cobertas, reutilizando cenas quando possível. Entradas inativas também contam como intervalos atendidos. Se constantes ativos excederem **Maximum constant summary size**, **Updating continuity** (atualizando continuidade) combina após a resposta apenas seus textos, usando auxiliar e **Chat Summary → Maximum output size**. A substituição aparece em Chat Summaries; entradas substituídas ficam inativas e recuperáveis. Cenas permanecem no cofre. Uma falha preserva resumos; tente novamente com **Resume processing** (retomar processamento).

A recuperação fornece resumos relevantes e diálogos exatos com números originais e falantes. Cada resumo acompanha seu trecho com um único cabeçalho de intervalo. O bloco indica o intervalo atual e o número da última mensagem do usuário para distinguir passado e presente. Uma cena só é recuperada se todas as fontes estiverem fora do histórico enviado; trechos também excluem mensagens atuais. Resumos manuais contribuem apenas se todo o intervalo estiver arquivado. Um intervalo que cruza o corte não é repetido inteiro junto às mensagens atuais. Contam personas e personagens. A regeneração histórica usa fontes anteriores ao alvo, mesmo antes da janela gerenciada. Editar, trocar variante, ocultar ou excluir fontes exige reavaliar a memória derivada.

Abra **Access memories for this chat** no mesmo painel para pesquisar resumos de cenas numerados em ordem cronológica, inspecionar seus períodos e públicos, editar **Summary text** (texto do resumo) ou ler as mensagens originais completas com **Inspect source messages** (inspecionar mensagens de origem). Os trechos internos reproduzidos literalmente não são entradas separadas de resumo de cena. Os períodos da história fundamentados nas fontes acompanham o contexto recuperado; datas desconhecidas continuam desconhecidas. Você pode desativar registros de recuperação, reindexar, exportar/importar ou confirmar **Delete all memories** (excluir todas as memórias) para reiniciar a preparação da memória mantendo o chat original e as configurações. Correções dos resumos manuais originais são preservadas e invalidam a continuidade dependente. Desativar um registro é diferente de ocultar suas mensagens de origem: fontes ocultas são a autoridade para o conhecimento dos personagens.
...
| `recalled_messages` | Alias antigo da mesma seção conjunta Recalled Scenes. |

Todas as cenas recuperadas ficam em uma seção **Recalled Scenes**: cada resumo é seguido imediatamente pelo trecho da cena, se disponível. Um marcador `recalled_scenes` ativado tem prioridade sobre o antigo `recalled_messages`; eles nunca geram seções separadas.
