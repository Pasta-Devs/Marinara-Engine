...
2. **World.** Defina gênero, cenário, tom, dificuldade, classificação de conteúdo e idioma. Escolha **Classic** (combate por menus) ou **Tactical** (combate em grade) em **Combat Preference** (preferência de combate). Partidas Tactical também oferecem semente, tamanho e orientações de terreno opcionais. Se houver um conjunto de regras Game Mode instalado, **Rules** (Regras) aparece abaixo: mantenha **Marinara's own rules** ou escolha o conjunto. Veja [Escolher as regras](#choosing-rules).
...
O assistente distingue três caminhos: combate resolvido pelo próprio conjunto de regras, combate do Marinara usando a vida, a energia e as habilidades da ficha ou combate do Marinara conforme **Combat Preference**. Quando o conjunto resolve a luta, o menu mostra os ataques e as habilidades do personagem com seus custos. O painel apresenta a ordem dos turnos, a rodada, as condições e os recursos de ação restantes; o registro mostra o cálculo real. Cada ação é salva imediatamente na ficha, então fechar a aba durante o combate não perde o resultado. Depois, o GM é informado de que os números já foram resolvidos. Se o conjunto também define quanto vale uma casa em sua própria unidade de distância, **Combat Preference** decide a apresentação: **Classic** joga sem posições e **Tactical** usa um tabuleiro gerado onde você se move, mede alcance e distância nas unidades do conjunto e mira uma área em uma casa. Veja [Combate](combat.md#games-whose-ruleset-resolves-its-own-fights). Um conjunto sem distância mantém a preferência, mas não a usa.
...
**Finish rolled turns in one request** (Terminar turnos com dados em uma solicitação) muda o segundo tipo de chamada descrito acima. Um turno com rolagens normalmente custa duas solicitações: o rascunho e uma reescrita com os números reais. Com a opção ativa, o GM escreve o desfecho às cegas e o motor preenche os números depois, então o turno custa uma solicitação. Ela fica desativada por padrão. Duas coisas ainda acrescentam outra solicitação: chamar a ferramenta de dados com **Enable Tool Use** ativo e uma **Game tool connection** separada, que sempre faz seu próprio planejamento. Veja [Game Mode: Dados e testes de habilidade](dice-and-skill-checks.md#finishing-a-rolled-turn-in-one-request).
