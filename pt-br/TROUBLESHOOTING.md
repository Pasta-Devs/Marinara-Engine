...
Pare o servidor e rode `start.bat`, `start.sh` ou `start-termux.sh` de novo. O inicializador verifica o inventário da compilação do Vite, incluindo trechos de JavaScript carregados sob demanda, e recompila os arquivos incompletos do cliente antes de iniciar. Uma compilação anterior a essa verificação é refeita uma vez para criar o inventário. Se a compilação falhar, guarde o erro do terminal para pedir ajuda. Em uma instalação manual, rode `pnpm build` na raiz do repositório antes de iniciar novamente.

Se a verificação passar e a página continuar em branco, faça uma recarga forçada ou tente uma janela anônima. Informe a URL completa do arquivo que falhou, o status HTTP, Content-Type e a primeira linha da resposta, junto com a saída do inicializador. Arquivos ausentes agora retornam HTTP 404 em vez da página HTML do aplicativo.
...
Inicie Marinara com `start.bat`, `start-local.bat`, `start.sh`, `start-termux.sh` ou `pnpm start`. Esses comandos mantêm o servidor ligado ao seu inicializador e aguardam o processo antigo terminar antes de iniciar o substituto. O reinício pelo aplicativo fecha conexões persistentes após quatro segundos e força a saída após oito segundos se o encerramento continuar travado. Um encerramento forçado pode interromper gravações pendentes e é registrado como forçado nos diagnósticos. Execuções diretas com `node` e processos que monitoram arquivos durante o desenvolvimento não são substituídos automaticamente: pare e reinicie esses processos pelo terminal. O Docker continua usando a política de reinício do contêiner. No Windows, pressione Ctrl+C no console do inicializador e aguarde **Shutdown complete** (desligamento concluído) antes de fechá-lo. Fechar ou encerrar o terminal à força pode interromper salvamentos pendentes.
...
### Chrome no Android não oferece instalação de PWA com Basic Auth

Use um endereço HTTPS, entre na conta e recarregue após atualizar Marinara. O aplicativo solicita o manifesto com credenciais, então Basic Auth pode continuar ativado. Se a instalação ainda estiver indisponível, confira se o proxy entrega `/manifest.json` como JSON, e não como uma página de login ou um erro.
