...
- Ustaw **Maximum allowed context before compression (tokens)** (maksymalny kontekst przed kompresją) w granicach kontekstu modelu czatu. Limit obejmuje całe żądanie czatu i każde żądanie przetwarzania pamięci, z uwzględnieniem niższego limitu wybranego modelu. Wlicza szacowane tokeny promptu, narzędzia, załączniki, miejsce na odpowiedź i margines bezpieczeństwa; nie jest dokładnym licznikiem tokenów ani kosztów.
- Ustaw **Maximum constant summary size (tokens)** (maksymalny rozmiar stałych podsumowań) poniżej tego limitu. Dotyczy tylko aktywnych stałych podsumowań w **Chat Summaries** (podsumowania czatu), bez bieżących wiadomości, przywołanych fragmentów i podsumowań scen w archiwum. Stałe podsumowania o rozmiarze 8k przy limicie 10k nie wywołają łączenia z powodu dołączonych wiadomości.
...
- Wszystkie wywołania podsumowań pamięci, także scen i łączenia stałych podsumowań, używają **Chat Summary → Maximum output size** (maksymalny rozmiar odpowiedzi). Limit połączenia pomocniczego go nie zastępuje. Skonfigurowany limit kontekstu musi pomieścić wejście i tę rezerwę na odpowiedź.
- Żądanie podsumowania sceny zawiera instrukcje, uprawnione wiadomości tej sceny, pasujące poprawki z zakresami oraz format odpowiedzi JSON. Zbyt duże sceny są przetwarzane w zapisywanych partiach, a następnie łączone. Domyślny prompt tworzy historyczne streszczenie bez sekcji o bieżącej sytuacji i otwartych napięciach; własne prompty wybrane w **Summaries** nadal obowiązują. Advanced Memory działa niezależnie od głównego przełącznika Agents i nie wymaga pobierania agenta.
...
W istniejącym czacie kliknij najpierw **Prepare existing history** (przygotuj dotychczasową historię). Starsza historia jest przetwarzana partiami, a etap widać obok kołowrotka Professor Mari. **Cancel** (anuluj) zachowuje ukończoną pracę; **Resume** (wznów) kontynuuje po zamknięciu panelu, restarcie serwera lub aktualizacji. Błąd modelu zachowuje poprawną pamięć i pozwala ponowić próbę. Nie resetuj pamięci po błędzie: wznowienie wykorzystuje gotowe podsumowania i niezmienione wyniki wykrywania scen. Ostatnia trwająca scena pozostaje otwarta i otrzymuje podsumowanie po zakończeniu; gdy jej wiadomości przekroczą limit kontekstu, używane są ograniczone fragmenty źródłowe.
...
Główne generowanie od razu czyta zapisaną pamięć. Nigdy nie uruchamia generowania ciągłości ani na nie nie czeka, także gdy model pomocniczy pracuje w tle. Przy osiągnięciu limitu całego żądania starsze zakończone sceny opuszczają bieżące okno. Jeśli niedokończona scena lub zbyt duże stałe podsumowanie nie mieści się w limicie, żądanie używa jawnie oznaczonych fragmentów źródłowych i zachowuje najnowsze wiadomości. Dopasowanie nie nadpisuje zapisanych podsumowań ani oryginałów.

Po głównej odpowiedzi Advanced Memory rozszerza istniejące zakresowe **Chat Summaries** tylko o nieobjęte nimi wiadomości archiwalne, wykorzystując gotowe podsumowania scen, gdy to możliwe. Także nieaktywne wpisy oznaczają już obsłużone zakresy. Gdy aktywne stałe podsumowania przekroczą **Maximum constant summary size**, zadanie **Updating continuity** (aktualizowanie ciągłości) łączy po odpowiedzi tylko teksty podsumowań, używając modelu pomocniczego i **Chat Summary → Maximum output size**. Wynik trafia do Chat Summaries; zastąpione wpisy stają się nieaktywne i można je odzyskać. Podsumowania scen pozostają w skarbcu. Błąd łączenia zachowuje dotychczasowe podsumowania; ponów je przyciskiem **Resume processing** (wznów przetwarzanie).

Archiwum odzyskuje istotne podsumowania scen i dokładne dialogi z oryginalnymi numerami oraz mówcami. Każde podsumowanie jest przy swoim fragmencie, z jednym nagłówkiem zakresu. Blok wskazuje bieżący zakres historii i numer ostatniej wiadomości użytkownika, odróżniając przeszłość od obecnego momentu. Podsumowanie sceny jest przywoływane tylko wtedy, gdy wszystkie jego wiadomości są poza wysyłaną historią; fragmenty też wykluczają bieżące wiadomości. Ręczne zakresowe podsumowania wchodzą do ciągłości tylko w całości poza bieżącym oknem. Zakres przecinający granicę nie jest powtarzany w całości obok swoich bieżących wiadomości. Liczą się wiadomości person i postaci. Historyczna regeneracja używa źródeł sprzed odpowiedzi, nawet gdy poprzedza ona zarządzane okno. Edycja, zmiana wariantu, ukrycie lub usunięcie źródła powoduje ponowne sprawdzenie zależnej pamięci.

Otwórz **Access memories for this chat** w tym samym panelu, aby przeszukiwać chronologicznie ponumerowane podsumowania scen, sprawdzać ich ramy czasowe i odbiorców, edytować **Summary text** (tekst podsumowania) lub czytać pełne oryginalne wiadomości przez **Inspect source messages** (sprawdź wiadomości źródłowe). Wewnętrzne dosłowne fragmenty nie są osobnymi wpisami podsumowań scen. Przywoływany kontekst zawiera ramy czasowe fabuły oparte na źródłach; nieznane daty pozostają nieznane. Możesz wyłączać rekordy przywoływania, przebudować indeks, eksportować i importować lub potwierdzić **Delete all memories** (usuń wszystkie wspomnienia), aby zacząć przygotowanie pamięci od nowa, zachowując oryginalny czat i ustawienia. Poprawki oryginalnych ręcznych podsumowań zostają zachowane i unieważniają zależne podsumowanie ciągłości. Wyłączenie rekordu różni się od ukrycia wiadomości źródłowych: to ukrycie źródła rozstrzyga, co postać może wiedzieć.
