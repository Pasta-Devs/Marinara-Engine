...
- Marinara's hostable Rust runtime, `marinara-server`, running locally or on your network (default: `localhost:8787`)

Start the hostable runtime from the repo with:

```bash
cargo run --manifest-path src-tauri/Cargo.toml --bin marinara-server
```

The Tauri desktop app can still be open, but the Home Assistant integration talks to `marinara-server` over `/api/invoke`; it does not talk to the Vite/Tauri UI port.
...
3. Enter the host and port where `marinara-server` is running (default: `localhost` / `8787`)
4. Click **Submit**
...
Refactor compatibility service. If `user_message` is provided, it is stored in the target chat. The Tauri refactor does not expose the desktop generation loop through `marinara-server`, so open the chat in the desktop app to generate the response.
...
Make sure `marinara-server` is running and the host/port you entered match where it's listening (default: `localhost:8787`). The desktop UI/Vite port is not the Home Assistant API endpoint.
