# Developer foundations: what this branch adds

This page walks through the `contrib/dev-foundations` branch one piece at a time. Every piece builds on code that is already in Marinara Engine and already works well. Nothing here renames an upstream API, replaces the shared logger or changes a file's conventions. Where a piece changes behaviour for everyone, that is said plainly. Everything else is off until you turn it on.

The branch is five code commits on top of `staging` (`60ed7ec80`), in this order, followed by the docs commit that adds this page:

| Commit      | Piece                                                                    |
| ----------- | ------------------------------------------------------------------------ |
| `f69091359` | Test harness: isolated regression runs                                   |
| `dd5845a0a` | Logging: request id trail, startup timeline, one line per failure        |
| `e2fbe83b9` | Performance: two opt-in lorebook settings                                |
| `7b43489e6` | Robustness: five opt-in areas plus a read-only diagnostics route         |
| `12f4b2552` | Startup: early internal requests wait until route registration has ended |

The branch ends with one extra commit that adds a Dev MCP server under `tools/dev-mcp` for coding assistants working on Marinara locally. It is optional tooling: it lives outside the pnpm workspace and the Docker image, its README explains setup, and it relies on the request trail from the logging section to follow a request through the logs. Nothing in the app depends on it, so dropping that commit removes it cleanly.
...
Reverting: the commits are ordered, and later ones touch a few of the same files (`app.ts`, `index.ts`, `capability-module-runtime.service.ts`, `CHANGELOG.md`). Reverting from the newest commit backward avoids conflicts. Inside the performance and robustness commits every behaviour change is an environment setting that defaults to off, so "turn it off" normally means leaving the setting unset. The startup commit (section 5) has no setting; it is a fix and is always on.
...
Then look in your temp folder: no `marinara-regression-*` folder should be left. The full suite gave the same result before and after this commit on the test machine.

**How to turn it off / revert.** There is no switch; isolation is the default. `git revert f69091359` restores the old runner (revert the newer commits first; they only share `CHANGELOG.md` with it). If you would like an opt-out, something like `MARINARA_REGRESSION_INHERIT_STORAGE=1` would be a few lines; it was left out so as not to add settings nobody asked for.
...
**How to turn it off / revert.** `LOG_DISABLE_REQUEST_LOGGING=true` silences the request lines as before, and the default `LOG_LEVEL=warn` already hides the new info lines. To remove the whole piece, `git revert dd5845a0a` (after reverting the three newer code commits).

---

## 3. Performance

Both items are off by default. With them off, the prompt and the stored data are byte-for-byte what they are today. Both are documented in `.env.example` and in a new Lorebooks table in `docs/CONFIGURATION.md`.

### 3a. Stable lorebook group winners (`LOREBOOK_STABLE_GROUP_WINNERS`)
...
**How to check it yourself.** `node scripts/run-regressions.mjs --filter lorebook-group-seed`

**How to turn it off / revert.** Leave `LOREBOOK_STABLE_GROUP_WINNERS` unset or `false`. The code is in `e2fbe83b9`.
...
**How to turn it off / revert.** Leave `LOREBOOK_COMPACT_STORED_SCANS` unset or `false`. Messages already compacted stay compact (the newest message always keeps its text). If you ran the maintenance script with `--apply`, its table backups can restore the older shape. The code is in `e2fbe83b9`.

---

## 4. Robustness

Every behaviour change here is an environment setting that defaults to off, documented in a new Robustness table in `docs/CONFIGURATION.md` and in `.env.example`. The areas touch separate files, so each can be reviewed, split out or reverted alone. All code is in `7b43489e6`. New parameters and fields are optional, and `isRateLimitError` and `base-provider.ts` are untouched.
...
### 4d. Provider retry on transient network errors
...
**How to turn it off / revert.** Leave the setting unset.
...
**How to turn it off / revert.** There is no switch. `MARINARA_INJECT_DURING_REGISTRATION` is only the error code on `InjectDuringRegistrationError`, not a setting. To remove the fix, `git revert 12f4b2552`; it is the last server commit (only the docs and the independent Dev MCP commit come after it), so nothing else needs to be reverted first.

---

## Test results on the development machine

The full Node regression suite gave the same result with and without these commits. Four files fail identically on plain `staging` on the Windows test machine (`decision-sidecar-runtime`, `gallery-previews`, `request-timeouts`, `server-signal-shutdown`); their causes are local (a disk-space check for a model download, temp-folder `EPERM`, a `D:` ESM URL, Ctrl+C in PowerShell), not these changes. The other 372 of 376 pass. `tsc --noEmit` passes for shared, server and client. No paid model calls were made.
