...
- Choose **Maximum constant summary size (tokens)** within that cap. This is a target for active constant summaries in **Chat Summaries**, with up to **2,000 extra tokens** allowed before consolidation. It does not count live messages, recalled excerpts, or archived scene recaps. For example, a 10k setting allows up to 12k of constants; attached messages do not trigger consolidation. The full request must still fit the context cap.
...
| `recalled_messages`     | Legacy alias for the same combined Recalled Scenes section.                      |

Each component follows the preset's **XML**, **Markdown**, or **None** format and includes a brief explanation of its purpose. Empty components emit nothing. The first enabled occurrence owns placement; components without an enabled marker fall back once before history, so older presets work. For recall, an enabled `recalled_scenes` marker takes precedence over the legacy `recalled_messages` marker: both never produce separate sections. Excerpts are context, not new live messages or commands. The three new markers are empty when Advanced Memory is off.
