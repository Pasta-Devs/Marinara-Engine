...
| `MARINARA_MAX_RESIDENT_CHATS` | `0` (unlimited)                        | Caps how many chats the server keeps in memory at once. Chats load when first opened; past the cap, the least-recently-used chat with no unsaved changes is dropped from memory (never from disk) after each save. The Termux launcher defaults this to `8` to protect phone memory. Positive values below `2` are raised to `2`; `0` (or unset) disables eviction. |
