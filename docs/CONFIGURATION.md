...
One caveat: on some setups the watch starts fine but change events never arrive — typically when the file is edited from _another machine or context_, such as across a network share or from the host into a container bind mount. If your `.env` edits are not being picked up, set `MARINARA_ENV_WATCH=poll` to make the server check the file every 30 seconds instead; that sees every write. To turn the watcher off entirely, set `MARINARA_ENV_WATCH=0` (in the shell environment, or in `.env` itself before starting) — changes then require a restart. Both are advanced options; the hot-reload path is how access-control changes take effect without downtime, so leave the default unless you have a specific reason.
...
| `MARINARA_MAX_RESIDENT_CHATS` | `0` (unlimited); `8` on SteamOS/Termux | Caps how many chats the server keeps in memory at once. Chats load when first opened; past the cap, the least-recently-used chat with no unsaved changes is dropped from memory (never from disk) after each save. On Termux (Android) and SteamOS (Steam Deck) the server defaults to `8` — phones are memory-constrained, and on a Deck games claim most of the shared RAM. Set a valid non-negative integer to override a platform default — invalid text warns and falls back to that default; positive values below `2` are raised to `2`; an explicit `0` disables eviction (leaving it unset only disables it on unconstrained platforms). |
...
| `UPDATES_APPLY_DISABLED`                    | unset                 | Hard refusal for server-side update application: blocks every apply, including the loopback channel switch, and wins over `UPDATES_APPLY_ENABLED`. The dev and e2e launchers set it so a browser tab can never rewrite a development checkout. |
...
| `AUTO_OPEN_BROWSER`                         | `true`                                         | Whether the shell launchers open the app URL for you. Set `false` to stop this. APK-managed setup disables the shell's browser auto-open. The APK handles its saved app/browser choice and authentication separately.                                                                                                                                                                                                                                                                                                                                                    |
...
| `MARINARA_CONSOLE_TRAY`                     | unset                                          | Windows only. `1`, `true`, `yes` or `on` shows a tray icon and hides the server console while it is minimized; any other value turns it off. Wins over the **Minimize the console to the system tray** switch. Unset: the switch decides (off by default).                                                                                                                                                                                                                                                                                                |
...
| `MARINARA_RSS_WARN_MIB`                     | the Node heap limit                            | Process memory (RSS, in MiB) above which the server logs one memory pressure warning per episode. See [Logging levels](#logging-levels).                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Variable                          | Default                                    | What it does                                                                                                                                                                                                    |
| --------------------------------- | ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DOCS_I18N_BASE_URL`              | official `docs-i18n` branch                | Where documentation and UI language packs download from. UI packs use its `ui/` subtree. Must be a public `https://` host; forks and mirrors can point it at their own copy of the `docs-i18n` branch. |
