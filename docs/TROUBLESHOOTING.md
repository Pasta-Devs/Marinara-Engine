...
Start Marinara using `start.bat`, `start-local.bat`, `start.sh`, `start-termux.sh`, or `pnpm start`. These keep the server attached to its launcher and wait for the old process to exit before starting its replacement. In-app restart closes lingering connections after four seconds and forces exit after eight seconds if shutdown is still stuck; a forced shutdown can interrupt pending writes and is recorded as forced in diagnostics. Direct `node` runs and development watchers are not automatically replaced: stop and restart them from their terminal. Docker continues to use its container restart policy. On macOS, Linux, and Termux, closing the terminal window shuts the server down the same way Ctrl+C does: pending saves are written, the local-model sidecar is stopped, and the storage writer lease is released. On Windows, press Ctrl+C in the launcher console and wait for **Shutdown complete** before closing it. Force-killing the process can still interrupt pending saves.
