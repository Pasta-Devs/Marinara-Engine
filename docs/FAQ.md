...
A **storyboard** turns finished narration into keyframe images and optional animated clips. In Game Mode it plans one completed game master turn. In Roleplay it can automatically combine the ordered user and assistant messages since its previous successful run, based on an interval counted in completed assistant responses. The planner treats out-of-character and generation instructions as context instead of visible story events. Roleplay does not create a Game session or add an artificial transcript message.

Install Storyboard from **Agents → Download Agents**, then add it to a Game or Roleplay chat. It works independently and does not require Illustrator. Choose an image connection for stills and, if you want animation, a video connection. In either mode, you can make one by hand from the **Gallery** for the latest narration and reopen it with **View storyboard**. A manual Roleplay storyboard uses the latest user-and-assistant exchange; automatic Roleplay runs use the configured multi-response interval. Roleplay displays Storyboard media inline beneath its source message by default. **Chat Settings → Agents → Storyboards → Viewer Display** can switch it to the floating viewer or background layer.
