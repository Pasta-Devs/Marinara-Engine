...
## Proposed: health that is a track, not a number

Status: not built. Filed as #6407, with #6405 beside it. Raised by a community author building World of Darkness, Chronicles of Darkness, Exalted and Fate rulesets, who has a working overlay extension for several of them.

**What the format says today.** A `live.pools` entry is one number: a `max` that may be a value reference over a `derived` sum, an optional temporary buffer, and a value. That is the right shape for hit points, and it already covers a maximum that moves in play, because a pool's `max` is recomputed on every read. Chronicles' Stamina + Size works now:

```json
"derived": [{ "id": "health_max", "op": "sum", "of": [{ "abilityScore": "stamina" }, { "field": "size" }] }],
"live": { "pools": [{ "id": "health", "max": { "derived": "health_max" } }] }
```

**What it cannot say.** A `live.tracks` entry is `{ id, label, min, max, default? }`: a bounded integer with no per-step label and no per-step penalty. So the tracked systems cannot be written:

- **No wound penalties, and nothing reads a track.** Neither `difficultyLadder` (check difficulties) nor `proficiencyTiers` (skill training) reads live state, and no resolver applies a modifier from a pool or a track. A wound track cannot bite. The only lever is the Game Master typing `bonus="-2"` by hand inside the ruleset's declared `situationalDice` range, which turns a printed rule into the model's judgement.
- **No damage kinds on a track.** The `damage` op is arithmetic on one number, with no kind, no marks, no sorting and no upgrading. (`combat.damageTypes` exists, but only for resist, vulnerable and immune inside a fight; it never writes to the sheet.)
- **Health must be a pool.** `combat.health` names one `sheet.live.pools` entry, and the cross-check refuses a track id outright.

**The shape to build**, taken from that author's own bundles rather than invented. Their V20 and Exalted tracks are the same shape with different labels:
...
Two details carry their weight. `severity` orders the kinds, which is the whole of the placement rule: the marks on a track are held sorted with the most severe first, so a new mark takes the highest level its severity earns and pushes the lighter ones down the track, and "the lightest mark upgrades one step when a full track is marked again" becomes an ordering statement rather than special-case code. And `penalty` is per LEVEL rather than per mark, which is what makes the repeated labels meaningful.

Said exactly, because a plan that is vague here produces the wrong track: levels are written best first and marked from the top down; the penalty in force is the one on the LOWEST marked level, not the sum of the marked ones; a mark is placed in severity order among the marks already there, never appended; and marking a track that is already full upgrades its lowest-severity mark to the next severity up rather than adding a mark, with a track full of the highest severity being the end of it. Fixtures must cover a track carrying more than one kind at once, because that is where every one of those rules is decided.

So: `live.tracks[]` gains optional `levels` (1 to 16, in order, worst last) and `kinds` (1 to 6). The two words are kept apart on purpose: `kinds` is what the RULESET declares a track may take, and a mark is one of those kinds sitting on the track in play. The live state holds marks; the definition holds kinds. A track that declares `levels` holds a list of marks rather than one integer, kept sorted most severe first, each mark naming one of the declared `kinds`. `{ op: "damage", track, kind, amount }` marks it, where `amount` is a number of marks of that one kind, applied one at a time so that a track filling partway through is handled by the same rule as a track that was already full: each mark is placed in severity order while a level is free, and once none is, it upgrades the lowest-severity mark by one step instead of being added. A mark that would upgrade past the highest severity is kept at the highest, and the mark that could not land is counted on the live track as an overflow, persisted rather than only reported, because that is where these systems put death and a reload must not forget it. The step that caused it also emits an event, so a fight can say it out loud. Healing is the same op with a negative amount, clearing the lightest marks first. **The resolver reads the penalty**: dice off the pool for `dice-pool`, a flat modifier for `dice-sum`. That one wire is what makes the track a rule instead of a picture, and it is the part an overlay extension cannot do from outside the Engine. `combat.health` accepts such a track as well as a pool and marks it instead of subtracting. The sheet draws the boxes and lets the player mark and clear them by hand, because these systems expect a player to keep their own track. Refused at import for a resolution kind that cannot honour a penalty, and gated behind a Capability API bump so an older Engine refuses the package rather than drawing a track that does nothing.

**Beside it, #6405**: a catalog entry cannot change how a check is rolled, so a charm that re-rolls 1s or buys automatic successes is flavour text. The two are the same shape of gap, the sheet knowing a number the resolver will not read, and they should probably be built together.
...
| 9a  | Engine          | Wound tracks: `live.tracks[].levels` and `.marks`, the resolver reading the penalty, `combat.health` on a track (#6407)                                             | A `game-ruleset-wound-tracks.regression.ts` on a hand-drawn track: marks placed in severity order, a lighter mark pushed down, the penalty read off the lowest marked level, a full track upgrading its lowest mark, overflow at the highest severity, healing clearing the lightest first, and the penalty reaching a roll in both resolution kinds. A round trip through storage. A ruleset whose resolution cannot honour a penalty refused at import, and the Capability API number pinned. |
| 9b  | Engine          | A catalog entry that changes a check: `mechanics.check` and a ruleset-level spend (#6405)                                                                           | A check bought with a resource resolved in ONE step: the entry's effect applied to the roll the Engine is about to make and the cost paid from the same decision, with a pool too short leaving both undone. Pinned for a re-roll, for automatic successes and for the ruleset-level spend, on a `dice-pool` ruleset and refused on a `dice-sum` one.                                                                                                                                           |
